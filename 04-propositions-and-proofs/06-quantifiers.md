...
[Chapter 3, Section 1](../03-functions-and-structures/01-structure-basics.md).
It was already reused for `∧`
(the `⟨hp, hq⟩` of [Chapter 4, Section 5](05-and-or-not.md)), and here it builds an
...
- [`rfl`](https://lean-lang.org/doc/reference/latest/Tactic-Proofs/Tactic-Reference/) is the **proof**, filling in for `P 2`, i.e. for `2 % 2 = 0`. It
  ...
  evaluated, exactly like `2 + 2 = 4` back in [Chapter 4,
...
-- Chapter 5, "More tactics," alongside `unfold`.)
...
- The proof is not `rfl`. `p > 3 ∧ isPrime p` does not reduce to a plain
  ...
  brute-force tactic used by `not_example` in [Chapter 4, Section 5](05-and-or-not.md),
  which here checks `5 > 3` outright and tries every
...
stronger claim, $\forall n,\ \exists p > n,\ \mathrm{isPrime}\ p$ (a theorem
of Euclid, first proved around 300 BCE), needs an argument that works
uniformly for an *arbitrary* `n`. That means induction, not yet
introduced. [Chapter 5](../05-tactics/00-index.md) covers the tactics
that make an argument like that possible. A fully formalized proof of
this theorem lives in Mathlib as `Nat.exists_infinite_primes`, one of
the "Read more" pointers once [Chapter 14](../14-next-steps/00-index.md)
