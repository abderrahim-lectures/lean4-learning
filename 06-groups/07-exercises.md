...
3. *The three-line `#eval` computation in `perm3Group` showed
...
1. Build `boolXorGroup : Group Bool` where `op` is boolean XOR (`Bool.xor`),
   ...
   inverse). Proving each field using `by intro a; cases a <;> rfl` is
...
2. Verify on paper that `inv_left` and `inv_right` are genuinely
   ...
   the second theorem in Chapter 7 (left inverses equal right inverses).
