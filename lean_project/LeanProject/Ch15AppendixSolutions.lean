...
only referenced, not re-declared):
  ...
  - Chapter 7 exercise 1 (`boolXorGroup`): Ch07Groups.lean
  - Chapter 8 exercises 1-2 (`inv_inv`, `cancel_left`): Ch08GroupTheorems.lean
  - Chapter 9 exercise 3 (`mat2_not_comm`): Ch09Rings.lean
  - Chapter 12 exercise 1 (`CyclicArrow`/`cyclicQuiver`/`cPathAlpha`/
    `cPathBetaAlpha`/`cPathGammaBetaAlpha`) and exercise 2 (`append_nil_left`):
    Ch12PathAlgebras.lean

And one deliberate counterexample from the Appendix's Chapter 6 section
...
namespace Ch15AppendixSolutions
...
-- Ch07Groups.lean; the Appendix's explanation of why `inv_left`/`inv_right`
...
-- Ch08GroupTheorems.lean.

-- ── Chapter 9: rings ──────────────────────────────────────────────────

-- Exercise 1: `bool2CommGroup` / `bool2Ring` (Z/2Z on `Bool`). Uses
-- `boolXorGroup` from Ch07Groups.lean as the additive group.
...
-- Ch09Rings.lean; the Appendix's argument that `right_distrib` (not
...
-- in Ch10RingTheorems.lean, where the main chapter's `neg_one_mul` needed it.)
...
-- compiled verbatim in Ch12PathAlgebras.lean.
...
end Ch15AppendixSolutions

-- ── Appendix, Chapter 12 checkpoint project ───────────────────────────
...
-- `Ch15AppendixSolutions` above, because they extend the `Path` type (itself
-- declared at root in Ch12PathAlgebras.lean): dot notation (`p.length`)
-- resolves the field against the type's root namespace. Inside a wrapper
-- namespace the resulting `Ch15AppendixSolutions.Path.length` would not be
