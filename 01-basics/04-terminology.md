...
Chapter 3's logic, then real proofs — loose use of these words would
...
empty type — there's a unique, vacuous map out of it into anything).
...
| Symbol | Lean |
| --- | --- |
| `Ring` $\to$ `CommGroup` ("forgets $\cdot$") | `r.addGrp` for `r : Ring R` (the field holding the additive structure) |
| `CommGroup` $\to$ `Group` ("forgets commutativity") | `.toGroup`, generated by `CommGroup`'s `extends Group G` — so the full `Ring` $\to$ `Group` chain is `r.addGrp.toGroup` |
| `Group` $\to$ `Set` ("forgets $+$") | no field projection needed — `Group G`'s carrier *is* the type parameter `G` itself, not something extracted from a structure |
