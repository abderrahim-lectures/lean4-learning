...
`B : A → Prop`, as Chapter 3 makes precise. Every `∀ n : Nat, P n` written
...
The `Prop` type in Lean (Chapter 3) sits alongside `Type` as its own universe, with
...
$x \in A$, glued together as separate parts that never overlap. The Curry–Howard table in [Chapter 3,
Section 1](../03-propositions-and-proofs/01-prop.md) will
...
- **Constant family, varying index.** If $B(x)$ is the *same* type $B$
  ...
  exactly what `∧` in Chapter 3 will turn out to be. $P \wedge Q$ is
...
- **Two-point index, varying family.** If $A$ is a two-element type
  ...
  $P \sqcup Q$, the ordinary sum/coproduct type that `P ∨ Q` in Chapter 3
...
R) (hqr : Q → R) : R` shown in [Chapter 3, Section 5](../03-propositions-and-proofs/05-and-or-not.md) is the eliminator for `Or`, a case for `Or.inl` and a case for
...
3. `inductive` type declarations, in the specific extension used by Lean, the
   **Calculus of Inductive Constructions** (CIC), such as `Nat`, `Bool`,
   `Vec`/`Fin` (Section 3), `Path` (Chapter 11), and every `structure`
...
more than typing rules like the ones sketched here and in Chapter 5,
applied mechanically. [Chapter 5, Section 3](../05-rigor-check/03-typing-rules-and-safety.md)
...
- Pierce ([Pierce2002]) is cited here only by analogy. *Types and
  ...
  like `NatList = μX.⟨nil:Unit, cons:{Nat,X}⟩` (Ch. 20), plus Church
  encodings of algebraic datatypes (Ch. 5 untyped, §23.4 typed/System F).
