...
The progress and preservation theorems of Section 3 guaranteed that reduction
never changes the type of a well-typed term, but they said nothing about
when two terms should be considered *the same* term in the first place.
That is the last of the three questions of the chapter, and the sharpest one.
...
proposition you prove. There is no term of type `a ≡ b`. Either the
kernel of Lean confirms it during elaboration (silently, whenever it needs to
...
--                                      -- unknown `n`. Needs induction — Chapter 5.
```
...
determine whether `0 + n` reduces at all, Lean examines the outermost
shape of `n` (its WHNF). If `n` is an unknown variable rather than a known
`zero`/`succ` constructor, there is nothing to see, so no reduction fires
and the goal is stuck at `0 + n`, exactly as Chapter 5 found.
...
from Chapter 5 proves a propositional equality that
...
induction Chapter 5 walked through).
...
1. **`rfl` only ever proves definitional equalities.** When `rfl` fails,
   ...
   This is exactly the "failure is information" point from Chapter 5.
...
2. **[`rw`](https://lean-lang.org/doc/reference/latest/Tactic-Proofs/Tactic-Reference/) works up to propositional equality, but the resulting
   ...
   not type correct" error mentioned in Chapter 5, which is exactly a
   ...
   `Path` type of Chapter 12).

### A note on proof irrelevance

One more fact worth having, since it explains why this book never worries
about "which proof" fills a `Prop`-valued field, is that Lean treats all proofs of
...
provably equal to it. This is exactly what makes the `Mat2.mk.injEq`-based
extensionality reasoning of Chapter 9 work, and [`congr 1`](https://lean-lang.org/doc/reference/latest/Tactic-Proofs/Tactic-Reference/) in Chapter 11, which splits a
...
> Read more. The TPiL chapter on structures discusses eta for structures
> directly; the "Why bundle proofs with data at all?" discussion in
> [Chapter 7, Section 6](../07-groups/06-why-bundle.md) is the payoff this
...
- **Propositional equality.** "In other situations ... reductions
  ...
  (a proof) establishes that. It is not obvious just from staring at the
