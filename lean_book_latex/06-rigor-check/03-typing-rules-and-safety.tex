...
\textbf{Definition.} A \emph{typing judgment} \(\Gamma \vdash t : \tau\) asserts that in context \(\Gamma\), term \(t\) has type \(\tau\). (\cite{Pierce2002}, Ch. 9)
...
Without this clause, universally quantified statements over infinite types would live in \texttt{Type 1} instead of \texttt{Prop}, breaking the framework.

The \(j = 0\) case is what makes \texttt{∀} usable at all, not a footnote. When \(B\) lands in \texttt{Prop}, the whole Π-type is a \texttt{Prop} \emph{regardless of how large \(A\) is}. This is the \textbf{impredicativity of \texttt{Prop}}, and it is why \texttt{∀ n : Nat, n ≥ 0} is a proposition you can prove rather than an inhabitant of \texttt{Type 1}.

\textbf{Definition.} A sort is \emph{impredicative} if quantifying over its inhabitants can produce an element of the same or smaller sort.
...
\item
  Milner (\cite{Milner1978}, §2) covers the theoretical background for why STLC alone cannot type polymorphic functions like \texttt{identity}.
\item
