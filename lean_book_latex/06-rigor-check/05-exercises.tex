...
\item
  \emph{\texttt{Type : Type} would make the type system of Lean itself inconsistent. Given that, why does \texttt{Type 1 : Type 2}, \texttt{Type 2 : Type 3}, and so on not cause the exact same problem one level up?} Because the paradox needs a type containing \emph{itself}. \texttt{Type i : Type (i+1)} is always one level higher than what it classifies, so no universe ever contains its own code. The hierarchy is infinite specifically so this containment can never close the loop.
\item
  \emph{\texttt{rfl} and \texttt{decide} both look like they ``just work'' without argument. Are they doing the same thing?} No.~\texttt{rfl} checks that two terms reduce to an identical normal form. \texttt{decide} runs the algorithm of a \texttt{Decidable} instance to compute \texttt{true}/\texttt{false} outright. They overlap on simple closed arithmetic (both close \texttt{2 + 2 = 4}), but only \texttt{decide} can settle something like \texttt{¬ (3 ∣ 10)}, which is not a reduction question at all.
\item
...
Solutions: \hyperref[sec:15-appendix-solutions:05-chapter-6:1]{Appendix, Chapter 6}.
