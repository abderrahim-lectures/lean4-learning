...
\textbf{Claim.} (\cite{DummitFoote2003}, pp.~17-18) If \texttt{e' : G} also satisfies \texttt{∀ a, Grp.op e' a = a}, then \texttt{e' = Grp.id}.
...
\subsection{Informal vs.~formal: identity uniqueness}\label{sec:08-group-theorems:02-theorem-1:2}

\begin{quote}
\textbf{Informal proof.} ``Suppose \(e\) and \(e'\) are both identities. Then \(e' = e' \cdot e = e\).''
\end{quote}
...
theorem id_unique (e' : G) (h : (*$\forall$*) a : G, Grp.op e' a = a) : e' = Grp.id := by
  have step1 : Grp.op e' Grp.id = Grp.id := h Grp.id
  -- apply h at a := Grp.id to get e' (*$\cdot$*) e = e
  have step2 : Grp.op e' Grp.id = e' := Grp.id_right e'
  -- use the right-identity axiom to get e' (*$\cdot$*) e = e'
  rw [(*$\leftarrow$*) step2]
  -- rewrite right-to-left: replace e' with e' (*$\cdot$*) e in the goal
  exact step1
  -- the goal is now e' (*$\cdot$*) e = e, which is exactly step1
\end{lstlisting}

The informal proof is one sentence. The Lean version is four lines, but the logical shape is identical: both compute \(e' \cdot e\) two different ways and chain the two equalities. The difference is that Lean demands each step be a named \texttt{have} or a tactic, not a passing phrase.
\end{quote}
