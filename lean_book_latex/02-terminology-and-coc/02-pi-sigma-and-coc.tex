...
Five lines print, one per \texttt{succ} layer \texttt{Nat.rec} peels off \texttt{5 = succ(succ(succ(succ(succ zero))))}, each showing the running total \emph{before} the final \texttt{+ 2}. That total is \texttt{0}, then \texttt{2}, then \texttt{4}, then \texttt{6}, then \texttt{8}, then the returned \texttt{10}. Nothing about \texttt{Nat.rec} was changed to make this possible. The trace prints from inside the ordinary succ-case function supplied as the second argument of \texttt{Nat.rec}\allowbreak{}, the same function already described above.
...
\item
  \textbf{Proof irrelevance.} ``If \texttt{p : Prop} is any proposition, the Lean kernel treats any two elements \texttt{t1 t2 : p} as being definitionally equal \ldots{} This is known as proof irrelevance'' (\cite{TPIL4}, §3.1). Picture it like this. A ``verified'' stamp on a passport. Customs doesn't care which officer stamped it or how the check was carried out, only that a valid stamp exists. Any two proofs of the same \texttt{P : Prop} are just as interchangeable. For the metatheory underneath this working description, normalisation and decidable conversion for a universe hierarchy with a proof-irrelevant type of propositions ``close to the type system used in the proof assistant Lean'' is established directly in \cite{Coquand2021}.
\item
