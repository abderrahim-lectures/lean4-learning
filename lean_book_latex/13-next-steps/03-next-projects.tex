...
\tightlist
\item
  Read \texttt{Mathlib.Combinatorics.Quiver.Basic}'s \texttt{Quiver} class (already introduced in Chapter 11, Section 3's ``Mathlib equivalent'' box) and \texttt{Mathlib.CategoryTheory.Quiver}'s extension of it with identities and composition, comparing field-for-field against this book's plain \texttt{Quiver}/\texttt{Path}.
\item
...
\item
  ...
  m\, f\,(n\, f\, x)\) --- ``apply \(f\), \(m\) more times, starting from where \(n\) has already applied \(f\), \(n\) times,'' the same recursive shape that makes \texttt{Nat.add} recurse on its second argument (Chapter 4).
\item
