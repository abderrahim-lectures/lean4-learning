...
\item
  ...
  m\, f\,(n\, f\, x)\), meaning ``apply \(f\), \(m\) more times, starting from where \(n\) has already applied \(f\), \(n\) times.'' This is the same recursive shape that makes \texttt{Nat.add} recurse on its second argument (Chapter 4).
\item
