...
\item
  Girard, \emph{``Interprétation fonctionnelle et élimination des coupures dans l'arithmétique d'ordre supérieure,''} Thèse d'État, Université Paris VII, 1972 (not yet in this book's bibliography) --- the actual source of the \texttt{Type : Type} inconsistency: the proof that a calculus with the rule \texttt{⊢ * : *} loses the normalization property. \cite{Girard1971} (the 1971/1970 ``Une extension de l'interprétation de Gödel à l'analyse'' paper, already in this book's bibliography) is a different, earlier paper and is not that source. Thierry Coquand's 1986 paper ``An analysis of Girard's paradox,'' LICS 1986, is the standard modern exposition, also not yet cited here.
\end{itemize}
