...
Nothing about the \emph{mathematical content} changes between the two styles. The axioms of \texttt{Group} are the axioms, in either encoding. The type-class version also usually bundles in the \texttt{*}, \texttt{⁻¹}, \texttt{1} of the group as genuine \emph{notation} (through further, separate mechanisms, the \texttt{Mul}, \texttt{Inv}, \texttt{One} type classes that \texttt{Group} extends), so that Mathlib code reads as \texttt{a * b} instead of \texttt{Grp.op a b}. The suggested projects of Chapter 13 include redoing \texttt{Group}/\texttt{Ring} from this book as type classes once the reader is comfortable doing so. At that point, everything in this section is the vocabulary needed.
