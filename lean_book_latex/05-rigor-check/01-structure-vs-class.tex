...
Lean records the group structure of \texttt{Int} in a global table. From then on, any function that takes an \emph{implicit} \texttt{[Group G]} argument (square brackets, not curly braces; this is a third kind of argument, an \textbf{instance argument}) has it filled in automatically whenever \texttt{G} is unified with \texttt{Int}, with no explicit term needed at the call site.
