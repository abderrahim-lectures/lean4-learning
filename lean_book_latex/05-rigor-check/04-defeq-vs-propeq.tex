...
\tightlist
\item
  \textbf{Definitional equality.} ``Two terms that reduce to the same value are called definitionally equal. They are considered `the same' by Lean's type checker'' (\cite{TPIL4}, ``Dependent Types''). Brief: \texttt{rfl} succeeds exactly when this holds.
\item
  \textbf{Propositional equality.} ``In other situations \ldots{} reductions hold only propositionally, which is to say, they are equational theorems that must be applied explicitly'' (\cite{TPIL4}, ``Dependent Types'' / ``Tactics''). Brief: an ordinary \texttt{Prop} that must be proved; definitional equality implies propositional equality, but not conversely.
\item
