...
\textbf{Definition.} A \emph{proof} of \texttt{P : Prop} is a term \texttt{p : P}. The proposition is true exactly when such a term exists.

\textbf{Definition.} The \emph{Curry--Howard correspondence} identifies propositions with types and proofs with terms.
...
A Python function that needs a precondition to hold usually states it with \texttt{assert}.
...
That \texttt{assert} is a promise, not a proof. It says ``this had better be true,'' and finds out whether it actually was only at the moment \texttt{divide} runs, on whichever inputs happen to reach it in production. A caller three modules away that forgot to check for zero first gets a crash discovered by a user, not by a type checker. In Lean, the corresponding function does not assert \texttt{b ≠ 0}. It \emph{requires} a proof of \texttt{b ≠ 0} as an ordinary argument, and Curry-Howard is exactly what makes that possible, since \texttt{b ≠ 0} is a type and a proof of it is a term of that type, checkable the same way any other argument is checked.

\begin{lstlisting}
def safeDivide (a b : Nat) (h : b (*$\neq$*) 0) : Nat := a / b
\end{lstlisting}

Calling \texttt{safeDivide 10 0 proof} for some \texttt{proof : (0 : Nat) ≠ 0} cannot type-check, because no such proof exists to supply, \texttt{0 ≠ 0} being false. The precondition is not documentation and not a runtime check racing against whatever inputs show up first. It is part of the type of \texttt{safeDivide} itself, verified once, for every call site, before the program runs at all.
...
\item
  Howard (\cite{Howard1980}, pp.~479-490) is the original source of the correspondence this section is named for. Per Sørensen \& Urzyczyn, \emph{Lectures on the Curry-Howard Isomorphism}, Studies in Logic and the Foundations of Mathematics vol.~149, Elsevier, 2006 (a secondary source corroborating this history, not the paper by Howard itself), the manuscript by Howard was ``privately circulated'' from 1969 and not formally published until 1980, in a Festschrift for Curry. It develops the proofs-as-terms correspondence for implicational logic, extends it to the other propositional connectives, then to a term language for Heyting Arithmetic.
\end{itemize}
