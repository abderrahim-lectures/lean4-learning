\section{And, Or, Not}\label{sec:04-propositions-and-proofs:05-and-or-not:1}
...
\tightlist
\item
  \texttt{∧} (And) is a structure with two fields \texttt{left} and \texttt{right}. \texttt{⟨hp, hq⟩} is the same ``here are the pieces, in order'' anonymous-constructor shorthand from \hyperref[sec:03-functions-and-structures:01-structure-basics:1]{Chapter 3, Section 1}. Since the goal is \texttt{P ∧ Q}, Lean infers that \texttt{⟨hp, hq⟩} must mean ``build the \texttt{And} from a proof of \texttt{P} and a proof of \texttt{Q},'' in that order, with no need to spell out \texttt{And.intro hp hq}.
\end{itemize}
...
\tightlist
\item
  \texttt{absurd \{P Q : Prop\} (h1 : P) (h2 : ¬P) : Q} derives \emph{anything at all} from a genuine contradiction, a direct proof of \texttt{P} together with a proof that \texttt{P} is impossible. \texttt{anything\_\allowbreak{}from\_\allowbreak{}contradiction} shows this concretely. From \texttt{1 = 2} and \texttt{1 ≠ 2} (contradictory hypotheses that could never both hold, but which Lean happily accepts as \emph{given} hypotheses in a signature. Nothing prevents assuming something false; it only prevents \emph{proving} it from nothing), one may conclude literally any proposition \texttt{P} whatsoever. This is the ``ex falso quodlibet'' principle made concrete. This is \(\bot\)-elimination from \hyperref[sec:04-propositions-and-proofs:02-logic-recap:1]{Section 2}, which is valid in both classical and intuitionistic logic, so nothing classical is being smuggled in here. Once a contradiction is present among the hypotheses, the goal being proved stops mattering.
\end{itemize}
...
\subsection{Sources, quoted}\label{sec:04-propositions-and-proofs:05-and-or-not:2}
...
\tightlist
\item
  \textbf{Product.} ``A product of the family \((P_i)_{i \in I}\) is a pair \(\left(\prod_{i \in I} P_i, (p_i)_{i \in I}\right)\)'' together with projection morphisms \(p_j\) to each \(P_j\), such that any other candidate pair factors uniquely through it (\cite{Agore2023}, Definition 2.1.1). Picture it like this. \texttt{and\_\allowbreak{}left}'s \texttt{h.left} is exactly one of those projections \(p_j\), applied to the two-object family \(\{P, Q\}\).
\item
  \textbf{Coproduct.} Defined dually to the product, by reversing every arrow (\cite{Agore2023}, Definition 2.1.2, via the duality principle of Propositions 2.1.3--2.1.4). Picture it like this. \texttt{Or.inl}/\texttt{Or.inr} are the two injections into the coproduct, and \texttt{or\_\allowbreak{}comm\_\allowbreak{}term}'s use of \texttt{Or.elim} is the universal factoring property, reversed.
\item
  \textbf{Initial object.} An object \(0\) such that for every object \(C\) there is a unique morphism \(0 \to C\) (\cite{Agore2023}, Proposition 1.4.3). Picture it like this. \texttt{False} has no constructors, so vacuously there is exactly one function out of it into anything, which is why \texttt{¬P := (P → False)} and \texttt{absurd} can conclude \emph{any} \texttt{P} once a proof of \texttt{False} is in hand.
\end{itemize}
