...
The result is \textbf{not} simply one of the two inputs, the way \(K\)'s result always is. \(K\,a\,b \to_\beta a\) --- \(K\) \emph{discards} its second argument and returns the first, verbatim. Here, \((\lambda x.\lambda y.\, y\, x)\, a\, b \to_\beta b\, a\) --- both arguments survive, but with the \emph{second} one applied to the \emph{first}. This term is sometimes called the ``Thrush'' combinator (\(T\), satisfying \(T\, x\, y = y\, x\)): it flips the order of application rather than discarding anything, which is genuinely different behavior from \(K\), not just a relabeling of it.
...
Unlike \texttt{Vec}, \texttt{List} does have a \texttt{Repr} instance for any printable \texttt{α}, so \texttt{Vec.toList}'s recursion can be watched directly with \texttt{dbg\_\allowbreak{}trace}, using \texttt{Vec.replicate} (Chapter 1, Section 3) to build a concrete input:
