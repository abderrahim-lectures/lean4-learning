...
\] It is a \emph{magma} (a set with a binary operation and no axioms) signature, not yet a group; the axioms below cut out the genuine group structures as a subset.
...
\subsection{Informal vs.~formal: what ``a group'' means to each}\label{sec:07-groups:02-translating:2}

\begin{quote}
\textbf{Informal proof.} ``A group is a set \(G\) with \(\cdot\) satisfying associativity, identity, and inverses.''
\end{quote}
...
\begin{lstlisting}
structure Group (G : Type) where
  op : G (*$\rightarrow$*) G (*$\rightarrow$*) G              -- binary operation
  id : G                      -- identity element
  inv : G (*$\rightarrow$*) G                 -- inverse function
  assoc : (*$\forall$*) a b c : G, op (op a b) c = op a (op b c)
  -- associativity: (a (*$\cdot$*) b) (*$\cdot$*) c = a (*$\cdot$*) (b (*$\cdot$*) c) for all a, b, c
  id_left : (*$\forall$*) a : G, op id a = a
  -- left identity: e (*$\cdot$*) a = a for all a
  id_right : (*$\forall$*) a : G, op a id = a
  -- right identity: a (*$\cdot$*) e = a for all a
  inv_left : (*$\forall$*) a : G, op (inv a) a = id
  -- left inverse: a(*${}^-$*)(*\textsuperscript{1}*) (*$\cdot$*) a = e for all a
  inv_right : (*$\forall$*) a : G, op a (inv a) = id
  -- right inverse: a (*$\cdot$*) a(*${}^-$*)(*\textsuperscript{1}*) = e for all a
\end{lstlisting}

The informal version names three ingredients and three axioms. The Lean version lists \emph{eight} fields: three pieces of data plus five proof obligations (identity is split into left/right, inverse is split into left/right). A term of type \texttt{Group G} cannot exist unless all five proofs are supplied, so every theorem later in the book that takes a \texttt{Group G} gets those five facts for free, checked once, rather than trusting each caller.
\end{quote}
