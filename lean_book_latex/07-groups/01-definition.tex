...
A \texttt{Monoid} (Chapter 6) has an associative operation and a two-sided identity, nothing more; an element need not be undoable. What is the weakest extra requirement that makes every element undoable? Not ``some element cancels it'' --- that already holds vacuously for the identity itself paired with \(e \cdot e = e\). The requirement has to hold for \emph{every} element, and it has to be witnessed, not merely asserted: for each \(a\) there must exist \(a^{-1}\) with \(a \cdot a^{-1} = a^{-1} \cdot a =
