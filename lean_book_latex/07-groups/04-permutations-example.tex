...
\texttt{Perm3} (with composition) is \(S_3\), the symmetric group on three letters. It is the smallest non-abelian group (order \(6\)) and the standard first example in any course covering non-commutative groups. \texttt{swap01} and \texttt{cycle012} here are, respectively, a transposition and a 3-cycle. Together they generate all of \(S_3\), exactly as in the usual presentation \(S_3 = \langle r, s \mid r^3 = s^2 = e,\ srs
= r^{-1} \rangle\), with \(r\) = \texttt{cycle012} and \(s\) = \texttt{swap01}.
