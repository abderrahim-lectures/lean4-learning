...
\textbf{Definition.} A \emph{constructor} of a \texttt{structure} is the function that builds an element from its fields.

\textbf{Definition.} The \emph{anonymous constructor} \texttt{⟨e₁, e₂, ...⟩} applies the appropriate constructor without naming it.

\textbf{Definition.} \emph{Field projection} \texttt{.field} is shorthand for \texttt{S.field s}.
...
\texttt{shift} shows a structure used on \emph{both} sides of a function. It takes a \texttt{Point} in (reading its fields back out with the same \texttt{p.x}/\texttt{p.y} projection notation) and builds a new one via \texttt{\{ x := ..., y := ... \}}, the same field-naming syntax used above by \texttt{origin}.
...
A Python programmer reaching for the same ``bundle a couple of values together'' idea usually writes a \texttt{dict} first.
...
Nothing about \texttt{origin} says which keys it is supposed to have. A typo in a key name, \texttt{origin["ix"]}, is caught the same way a missing key is, by crashing at run time, on whichever run happens to reach that line first. \texttt{Point.x} has no such failure mode. \texttt{origin.z} is rejected while reading the file, before \texttt{\#eval} or any test ever runs, because \texttt{z} is not a field \texttt{structure Point} declares. A \texttt{@dataclass} closes part of this gap, since \texttt{dataclass} at least fixes the field names in advance, but a plain Python \texttt{dataclass} is mutable by default. \texttt{origin.x = 99} silently overwrites the field in place, and any other code still holding a reference to \texttt{origin} sees the change too, whether it wanted to or not. \texttt{shift} above does not do this. It builds a brand new \texttt{Point} and returns it, leaving the \texttt{Point} passed in exactly as it was. Reasoning about \texttt{shift p 3 4} never requires asking who else might be holding onto \texttt{p} and whether they will be surprised by it changing underneath them, a question that has to be asked constantly about shared mutable Python objects.
...
Here is the universal property itself, in the notation of this box. The unique \(h\) making both triangles commute is exactly what \texttt{\{ x := ..., y := ... \}} constructs when \(f\) and \(g\) are the two argument expressions of \(p\).

\begin{center}
\input{diagrams/categorical-product.tex}
\end{center}

\subsection{Sources, quoted}\label{sec:03-functions-and-structures:01-structure-basics:2}
...
\tightlist
\item
  \textbf{Record / tuple.} ``The simplest of these is pairs, or more generally tuples, of values \ldots{} the generalization from n-ary tuples to labeled records is equally straightforward'' (\cite{Pierce2002}, §11.6--§11.8). Picture it like this. An unlabeled tuple is a box packed in a fixed order. You just have to remember ``first slot is the name, second is the age.'' A labeled record is the same box with every slot tagged directly, ``Name: \textbf{\emph{, Age: }}.'' The \texttt{structure} construct in Lean is exactly this tagged version.
\item
...
\item
  The Lean 4 documentation (\cite{LeanDocs}) covers the constructor/projection/anonymous-constructor mechanics described above.
\end{itemize}
