...
\tightlist
\item
  \texttt{op : G → G → G} is the group operation, curried (Chapter 2).
\item
...
\begin{quote}
Read more. The actual \texttt{Group} in Mathlib (\texttt{Mathlib.Algebra.Group.Defs}) is a \texttt{class}, not the plain \texttt{structure} used in this book. It inherits from a chain of smaller classes (\texttt{Mul}, \texttt{One}, \texttt{Inv}, \texttt{Monoid}, \ldots) instead of listing all axioms in one place. See \hyperref[sec:13-next-steps:02-moving-to-mathlib:1]{Chapter 13} for the bridge between the two styles, and \hyperref[sec:05-rigor-check:01-structure-vs-class:1]{Chapter 5, Section 1} for why this book delays that mechanism.
\end{quote}
