...
\texttt{Perm3.ext} is a small helper (an \emph{extensionality} lemma, in the same spirit as the \texttt{Mat2.mk.injEq}-based reasoning of Chapter 8). Two \texttt{Perm3}s are equal exactly when their \texttt{toFun}s agree everywhere \emph{and} their \texttt{invFun}s agree everywhere, since those are the only two fields of \texttt{Perm3} (the proof fields do not matter, by proof irrelevance, Chapter 5). Each \texttt{Group} field is then proved by reducing to this extensionality check and confirming both functions involved agree pointwise. Most cases are \texttt{rfl} directly (both sides compute to the same function once unfolded), and \texttt{inv\_\allowbreak{}left}/ \texttt{inv\_\allowbreak{}right} cite exactly \texttt{f.left\_\allowbreak{}inv}/\texttt{f.right\_\allowbreak{}inv}, the proof obligations already bundled into \texttt{Perm3} itself.
