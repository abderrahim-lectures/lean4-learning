...
\item
  \emph{\(a \cdot 0 = 0\) is proved from distributivity and cancellation, not assumed. Could a ring exist where this fails?} Not among the \texttt{Ring}s of Lean. The proof uses only \texttt{left\_\allowbreak{}distrib} and the additive group axioms, which every \texttt{Ring} supplies by definition, so the conclusion is forced. A structure lacking it would simply not satisfy the fields of \texttt{Ring} itself, whatever else it looked like.
\item
...
Solutions: \hyperref[sec:15-appendix-solutions:09-chapter-10:1]{Appendix, Chapter 10}.
