...
\textbf{Finding the proof.} In ordinary notation, the standard trick is as follows. Memorize this as a pattern applicable to any additive-identity argument: start from \(0 = 0 + 0\), multiply through, and cancel.
...
The tactic steps in \texttt{mul\_\allowbreak{}zero} carry out exactly the paper derivation. \texttt{h0} states \texttt{0 + 0 = 0} (the identity padded with itself). \texttt{h1} applies \texttt{left\_\allowbreak{}distrib} to rewrite \texttt{a * (0 + 0)} as \texttt{a*0 + a*0}, then \texttt{rw [h0]} at \texttt{h1} collapses \texttt{a*(0+0)} to \texttt{a*0} on the left side, giving \texttt{a*0 = a*0 + a*0} (i.e.~\(x = x + x\)). \texttt{h2} adds \(-x\) to both sides of that equation using \texttt{congrArg}, then the four \texttt{rw} lines apply \texttt{inv\_\allowbreak{}left}, associativity, \texttt{inv\_\allowbreak{}left}, and \texttt{id\_\allowbreak{}left} in sequence to cancel \(-x + x\) and \(x + (-x)\) down to \(0\), leaving \(0 = a*0\). The final \texttt{exact h2.symm} flips the equality to match the goal.
...
\] using \(0 = 0+0\) and \texttt{left\_\allowbreak{}distrib}; adding \(-x\) and cancelling gives \(0 = x\). Conceptually this says the map \(x \mapsto a\cdot x\) is a group homomorphism of \((R,+)\), and homomorphisms send the identity to the identity. \(0\) absorbs because multiplication distributes over addition in each argument.

\end{mathreading}

\textbf{Programmer note (\texttt{Float}).} \texttt{mul\_\allowbreak{}zero} is the kind of fact code using floating-point numbers would not test. \texttt{x * 0} is \texttt{0} for \texttt{Int}, but \texttt{Float} multiplication by zero can produce \texttt{NaN}.
