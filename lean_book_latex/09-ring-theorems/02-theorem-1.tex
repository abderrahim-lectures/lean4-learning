...
No \texttt{Ring} axiom directly states this. \href{https://loogle.lean-lang.org/?q=mul_zero}{\texttt{mul\_\allowbreak{}zero}} is not a field of \texttt{Ring} (Chapter 8); it must instead be \emph{derived} from \texttt{left\_\allowbreak{}distrib} plus group cancellation. The general recipe is that whenever a goal involves \texttt{0} (or any identity element) in a non-trivial way, attempt rewriting it as \texttt{0 + 0}. This is exactly the ``pad with the identity'' trick of \hyperref[sec:07-group-theorems:03-theorem-2:1]{Theorem 2 of Chapter 7}, but here it is applied to the input of the equation to be proved, rather than to the output.
