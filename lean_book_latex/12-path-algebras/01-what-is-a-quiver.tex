...
\textbf{Definition} (Quiver). A \emph{quiver} \(Q\) consists of vertices \(Q_0\), arrows \(Q_1\), and source/target functions \(s, t : Q_1 \to Q_0\) (\cite{AssemSimsonSkowronski2006}, Definition 1.1). It is a \emph{directed graph} under another name. This is the same notion as \href{https://loogle.lean-lang.org/?q=Quiver}{\texttt{Quiver}} in Mathlib (built here from scratch, following the ``no Mathlib'' policy of Chapter 1, rather than reusing the Mathlib version).
