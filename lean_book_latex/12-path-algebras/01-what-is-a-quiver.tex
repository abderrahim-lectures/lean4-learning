...
A \textbf{quiver} is exactly this: a set of vertices \(Q_0\), a set of arrows \(Q_1\), and two functions \(s, t : Q_1 \to Q_0\) recording each arrow's \textbf{source} and \textbf{target}. It is a \emph{directed graph} under another name. This is the same notion as \href{https://loogle.lean-lang.org/?q=Quiver}{\texttt{Quiver}} in Mathlib (built here from scratch, following the ``no Mathlib'' policy of Chapter 1, rather than reusing the Mathlib version).
