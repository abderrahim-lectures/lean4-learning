...
\mathrm{Vec}\,\alpha\,n\). The surface syntax \texttt{(x : A) → B x} in Lean \textbf{is} \(\prod_{x:A} B(x)\), and \texttt{∀} is the same construct specialized to \texttt{B : A → Prop}, as Chapter 3 makes precise. Every \texttt{∀ n : Nat, P n} written from that point on is already a Π-type in exactly this sense.
