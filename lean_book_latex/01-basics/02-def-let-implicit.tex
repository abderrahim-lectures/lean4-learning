...
\item
  A \textbf{fourth}, rarer kind, \textbf{strict implicit}, written \texttt{\{\{x : α\}\}} (Lean also accepts a Unicode spelling of the same double-brace pair, seen occasionally in Mathlib source), behaves like \texttt{\{\}} except that Lean defers solving it until an \emph{explicit} argument after it is actually supplied. It is a Mathlib idiom for keeping partially applied functions well behaved, and is named here so it is recognizable if encountered in Mathlib source.
\item
