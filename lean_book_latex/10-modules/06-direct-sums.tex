...
\tightlist
\item
  \textbf{Direct sum of modules.} ``Let \(M_1, \ldots, M_k\) be a collection of \(R\)-modules. The collection of \(k\)-tuples \((m_1, \ldots, m_k)\) where \(m_i \in M_i\) with addition and action of \(R\) defined componentwise is called the direct product of \(M_1, \ldots, M_k\) \ldots{} also referred to as the (external) direct sum \ldots{} denoted \(M_1 \oplus \cdots \oplus M_k\)'' (\cite{DummitFoote2003}, §10.3 ``Generation of Modules, Direct Sums, and Free Modules,'' p.~351, Proposition 5). D\&F's explicit universal-property discussion in this section covers free modules and tensor products, not direct sums specifically --- the universal-property framing of direct sums used elsewhere in this book is its own extension of the pattern.
\end{itemize}
