...
= (X(YZ))_{i\ell}\), whose per-entry form is a polynomial identity over the commutative ring \(\mathbb{Z}\), closed above by hand through \texttt{Int.add\_\allowbreak{}mul}/\texttt{Int.mul\_\allowbreak{}add}/\texttt{Int.mul\_\allowbreak{}assoc} and \texttt{add4\_\allowbreak{}reorder}, since this book's Mathlib-free development has no \texttt{ring} tactic available. The construction generalizes exactly the same way to \(M_n(S)\) for any commutative ring \(S\), entrywise reduction turns each ring axiom into an \(S\)-polynomial identity, and \(M_n(S)\) is noncommutative for \(n \ge 2\).
