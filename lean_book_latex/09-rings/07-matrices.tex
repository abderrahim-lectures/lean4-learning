...
\texttt{Mat2} is the free abelian group \(\mathbb{Z}^4\) \(M_2(\mathbb{Z}) \cong \mathbb{Z}^4\) on the four matrix entries. The extensionality lemma \texttt{Mat2.ext}, supplied by hand right alongside the structure, since almost every proof below needs it, says two \texttt{Mat2} values are equal exactly when all four entries match.
