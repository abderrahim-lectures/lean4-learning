...
\to_\beta b\, a\). Both arguments survive, but with the \emph{second} one applied to the \emph{first}. This term is sometimes called the ``Thrush'' combinator, it flips the order of application rather than discarding anything, which is genuinely different behavior from \(K\), not just a relabeling of it.

\textbf{Definition.} The \emph{Thrush combinator} \(T\) satisfies \(T\,x\,y = y\,x\). It is a \emph{combinator}: a closed \(\lambda\)-term with no free variables.
