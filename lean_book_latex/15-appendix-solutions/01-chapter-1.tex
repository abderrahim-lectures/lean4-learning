...
Yes. \texttt{∀ n : Nat, Nat} is a Π-type whose body happens not to mention the bound variable. Every ordinary function type is a Π-type in the degenerate case where the codomain is constant. ``Dependent'' describes the \emph{interesting} instances, where the codomain does mention the bound variable, not a separate kind of arrow from the Π-type.
