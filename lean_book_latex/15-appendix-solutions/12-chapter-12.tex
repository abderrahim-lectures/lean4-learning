\section{Chapter 12: Quivers and path algebras}\label{sec:15-appendix-solutions:12-chapter-12:1}

\textbf{1. Adding a cycle}

The \texttt{ExampleArrow} of the book is left untouched; the same construction is done here on a fresh \texttt{CyclicArrow}/\texttt{cyclicQuiver} rather than extending \texttt{ExampleArrow} with a \texttt{gamma} arrow, since introducing a cycle into the running example of the book itself would make later chapters that assume acyclicity (see the finiteness discussion of Chapter 12) inconsistent.
...
theorem append_nil_left {V A : Type} {Q : Quiver V A} {u v : V} (p : Path Q u v) :
  ...
  | nil =>
    ...
    -- (the `nil` case binds no fresh variable here (*\textemdash{}*) the vertex is already
    -- fixed by the type of p itself (*\textemdash{}*) so there is nothing to name.)
...
This is exactly what the editor shows in the \texttt{cons} case. The Lean Infoview lists every hypothesis the pattern match introduces, namely \texttt{Q : Quiver V A}, \texttt{h : Q.source a = v✝}, \texttt{h' : Q.target a = w✝}, \texttt{q' : Path Q u v✝} (an intermediate vertex introduced by the pattern match, not the fixed endpoint \texttt{v}), and the induction hypothesis \texttt{ih : (Path.nil u).append q' = q'}, above the line, with the goal \texttt{(Path.nil u).append (Path.cons a h h' q') = Path.cons a h h' q'} below it.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The Lean Infoview panel in VS Code, showing the tactic state for the cons case of the induction for append\_nil\_left: hypotheses V A : Type, Q : Quiver V A, u v vt wt : V, a : A, h : Q.source a = v, h\textquotesingle{} : Q.target a = wt, q\textquotesingle{} : Path Q u vt, ih : (Path.nil u).append q\textquotesingle{} = q\textquotesingle, and the goal (Path.nil u).append (Path.cons a h h\textquotesingle{} q\textquotesingle) = Path.cons a h h\textquotesingle{} q\textquotesingle.}]{../lean_book/15-appendix-solutions/../12-path-algebras/images/append-nil-left-infoview.png}}
\caption{The Lean Infoview panel in VS Code, showing the tactic state for the \texttt{cons} case of the induction for \texttt{append\_\allowbreak{}nil\_\allowbreak{}left}: hypotheses \texttt{V A : Type}, \texttt{Q : Quiver V A}, \texttt{u v vt wt : V}, \texttt{a : A}, \texttt{h : Q.source a = v}, \texttt{h' : Q.target a = wt}, \texttt{q' : Path Q u vt}, \texttt{ih : (Path.nil u).append q' = q'}, and the goal \texttt{(Path.nil u).append (Path.cons a h h' q') = Path.cons a h h' q'}.}
\end{figure}

This mirrors Path.append's own recursion, case for case. \texttt{Path.append} was defined by matching on its second argument, so \texttt{induction p} (which splits into cases on exactly that argument, generating the matching induction hypothesis \texttt{ih} in the \texttt{cons} case) unfolds the definition directly. In the \texttt{nil} case, both sides are \texttt{Path.nil u} by definition. In the \texttt{cons} case, unfolding the defining equation of \texttt{Path.append} turns the goal into \texttt{Path.cons a h h' (Path.append (Path.nil u) q') = Path.cons a h h' q'} (the \href{https://lean-lang.org/doc/reference/latest/Tactic-Proofs/Tactic-Reference/}{\texttt{show}} line makes this explicit rather than leaving it to elaboration), and \texttt{rw [ih]} completes the proof using the induction hypothesis.
...
Neither \texttt{Path.append} nor \texttt{Path.length} can be watched with \texttt{dbg\_\allowbreak{}trace} in isolation. A bare \texttt{Path} has no \texttt{Repr} instance, so only the composed \texttt{.length} call ever produces something \texttt{\#eval} can print. Tracing both at once shows \texttt{Path.append} finish building the whole composed path first, then \texttt{Path.length} walk it afterward, and the two recursions do not interleave, as shown below.
...
\texttt{Path.length} recurses on the same two constructors as \texttt{Path.append} itself (Sections 4-5). \texttt{nil} contributes \texttt{0}, and each \texttt{cons} adds one to the length of the shorter path it extends. The proof of \texttt{append\_\allowbreak{}length} mirrors Path.append's own recursion case for case, exactly as the project asked, but with one genuine surprise if \texttt{rfl} is tried first in either case. It fails. \texttt{Path} is an \emph{indexed inductive type} (an inductive type parameterized by extra indices, like \texttt{Path Q u v}), parameterized by both endpoints. Because of this, Lean compiles a match on an indexed family so that its defining equations reduce only through their auto-generated equation lemmas, not through plain iota-reduction, once an abstract path (\texttt{p}, \texttt{q'}) is involved. Concrete, fully closed paths like \texttt{pathAlpha} still reduce fine under \texttt{\#eval}, which is why the three checks above work directly. But the \emph{general}, universally-quantified theorem does not close by \texttt{rfl}. \texttt{simp only [Path.append, Path.length]} names exactly the two definitions being unfolded and nothing else, so it plays the same explicit role a \texttt{rw} would if these equations were reachable that way. It is not standing in for an unknown pile of simp lemmas, only for the two named here. This is the same kind of real, verified obstacle that \texttt{Perm3.ext} ran into in Chapter 7, Section 4, with core Lean lacking a \texttt{structure} extensionality lemma, now met again with indexed recursion instead.
