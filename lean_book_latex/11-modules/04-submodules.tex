\section{Submodules}\label{sec:11-modules:04-submodules:1}

Section 3 exhibited a concrete module, the canonical \(\mathbb{Z}\)-action on an abelian group, built from nothing but the group structure itself. With at least one genuine module now in hand, the natural next question is which subsets of a module inherit its structure. This is the module analogue of a subgroup, a subset closed under the same operations the whole module is built from.

\textbf{Definition} (Submodule). A submodule of \(M\) is a subset closed under \(+\), containing \(0\), and closed under the scalar action (\cite{DummitFoote2003}, §10.1, p.~337). In Lean, ``a subset closed under some operations'' is naturally a \texttt{structure} bundling a \emph{predicate} on \texttt{M} together with closure proofs. This is the same ``data + proofs'' idea as \texttt{Group}, just with the ``data'' being a property instead of an operation.
...
\Rightarrow r\cdot m \in N\). These are exactly the conditions for \(N\) to be an \(R\)-submodule, closed under the abelian-group operations and stable under scalars. So \(N\) inherits an \(R\)-module structure and the inclusion \(N \hookrightarrow M\) is \(R\)-linear. Categorically this is a subobject of \(M\) in \(R\text{-}\mathbf{Mod}\).

\end{mathreading}

\subsection{\texorpdfstring{Example: the submodule of even integers, as a \(\mathbb{Z}\)-submodule of \(\mathbb{Z}\)}{Example: the submodule of even integers, as a \textbackslash mathbb\{Z\}-submodule of \textbackslash mathbb\{Z\}}}\label{sec:11-modules:04-submodules:2}
...
\texttt{Submodule.span R s} is, by construction, the smallest submodule containing \texttt{s}. \href{https://loogle.lean-lang.org/?q=Submodule.subset_span}{\texttt{Submodule.subset\_\allowbreak{}span}} is the one fact that does all the work here, ``the generators are always members,'' proved once, generically, instead of the custom \texttt{zero\_\allowbreak{}mem}/\texttt{add\_\allowbreak{}mem}/\texttt{smul\_\allowbreak{}mem} triple of the book for this specific subset.

\subsection{Sources, quoted}\label{sec:11-modules:04-submodules:3}
...
\tightlist
\item
  \textbf{Submodule.} ``An \(R\)-submodule of \(M\) is a subgroup \(N\) of \(M\) which is closed under the action of ring elements, i.e., \(rn \in N\), for all \(r \in R, n \in N\)'' (\cite{DummitFoote2003}, §10.1 ``Basic Definitions and Examples,'' p.~337).
\end{itemize}
