...
\texttt{Submodule Rg Mod} is a submodule \(N \le M\), presented by its membership predicate \(\chi_N : M \to \mathrm{Prop}\) (a subset \(N = \{\,m \in M \mid \chi_N(m)\,\}\)) together with closure proofs: \(0 \in N\), \(m,n \in N \Rightarrow m+n \in N\), and \(r\in R,\, m\in N \Rightarrow r\cdot m \in N\). These are exactly the conditions for \(N\) to be an \(R\)-submodule: closed under the abelian-group operations and stable under scalars. So \(N\) inherits an \(R\)-module structure and the inclusion \(N \hookrightarrow M\) is \(R\)-linear. Categorically this is a subobject of \(M\) in \(R\text{-}\mathbf{Mod}\).
