\section{\texorpdfstring{Example: every abelian group is a \(\mathbb{Z}\)-module}{Example: every abelian group is a \textbackslash mathbb\{Z\}-module}}\label{sec:11-modules:03-z-module-example:1}
...
\texttt{natSmul Grp n m} is literally \(m + m + \cdots + m\) (\(n\) times). It is defined by recursion on \texttt{n}, in exactly the way \texttt{Nat.add} itself was defined (Chapter 5). A \texttt{dbg\_\allowbreak{}trace} in the succ case shows this iterated addition happening one term at a time, using \texttt{intGroup} (Chapter 7) as a concrete \texttt{Group Int}.
