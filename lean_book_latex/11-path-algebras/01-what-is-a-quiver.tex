...
This chapter's stated goal is the path algebra of a directed graph --- a construction that needs paths before it needs an algebra, and needs a graph before it needs paths. So the chapter starts at the bottom: the graph itself, stripped to the bare minimum of data a ``directed graph'' requires. Everything from here through the checkpoint project is built on top of exactly this one definition.
