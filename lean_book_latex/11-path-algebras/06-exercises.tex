...
\item
  Prove \texttt{theorem append\_\allowbreak{}nil\_\allowbreak{}left \{V A\} \{Q : Quiver V A\} \{u v\} (p : Path Q u v) : Path.append (Path.nil u) p = p} by induction on \texttt{p} (mirroring the structure of the recursion of \texttt{Path.append} itself), spelling out the base (\texttt{Path.nil}) and inductive (\texttt{Path.cons}) cases as in the \texttt{induction} examples of Chapter 4.
\item
  (Optional, harder) Sketch, in comments, no need to complete the Lean, what a \texttt{structure PathAlgebra (V A : Type) (Q : Quiver V A) (k : Type) (Rg : Ring k)} would need to contain to satisfy the fields of \texttt{Ring} from Chapter 8.
\end{enumerate}
