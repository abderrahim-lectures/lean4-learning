...
\item
  \emph{Theorem Proving in Lean 4} (\cite{TPIL4}), §3.1 ``Propositions as Types'' --- Lean's own documentation on \texttt{Prop}, proof irrelevance, and universes, verified verbatim: ``\texttt{Prop} is syntactic sugar for \texttt{Sort 0},'' ``Lean's kernel treats any two elements \texttt{t1 t2 : p} as being definitionally equal\ldots{} known as \emph{proof irrelevance},'' matching the presentation here.
\item
