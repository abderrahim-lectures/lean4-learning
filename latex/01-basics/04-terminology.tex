...
\bottomrule\noalign{}
\endlastfoot
\texttt{Ring} \(\to\) \texttt{CommGroup} (``forgets \(\cdot\)'') & \texttt{r.addGrp} for \texttt{r : Ring R} (the field holding the additive structure) \\
\texttt{CommGroup} \(\to\) \texttt{Group} (``forgets commutativity'') & \texttt{.toGroup}, generated by \texttt{CommGroup}'s \texttt{extends Group G} --- so the full \texttt{Ring} \(\to\) \texttt{Group} chain is \texttt{r.addGrp.toGroup} \\
\texttt{Group} \(\to\) \texttt{Set} (``forgets \(+\)'') & no field projection needed --- \texttt{Group G}'s carrier \emph{is} the type parameter \texttt{G} itself, not something extracted from a structure \\
\end{longtable}
