...
\begin{quote}
Read more: ``deduction theorem'' and ``\(\lambda\)-abstraction rule'' are two names for the same rule. ``Deduction theorem'' names the \(\Rightarrow\)-intro rule from natural deduction, stated in \hyperref[sec:03-propositions-and-proofs:02-logic-recap:1]{Chapter 3 §2}. ``\(\lambda\)-abstraction rule'' names that same rule's Curry--Howard reading as a Lean \texttt{fun}, part of the typed \(\lambda\)-calculus formalized in \hyperref[sec:05-rigor-check:03-typing-rules-and-safety:1]{Chapter 5 §3}.
\end{quote}
