...
Section 3's progress and preservation theorems guaranteed that reduction never changes a well-typed term's type --- but they said nothing about when two terms should be considered \emph{the same} term in the first place. That is the last of the chapter's three questions, and the sharpest one: Chapter 4 introduced \href{https://lean-lang.org/doc/reference/latest/Tactic-Proofs/Tactic-Reference/}{\texttt{rfl}} as ``the proof that both sides compute to the same thing,'' using it freely without asking exactly what ``compute to the same thing'' means, or whether it is the \emph{only} notion of equality Lean has. It is not. Lean has (at least) two, and confusing them is a common source of real confusion once proofs get more intricate. They are worth separating out clearly now.
...
\begin{quote}
Read more: TPiL's chapter on structures discusses eta for structures directly; the ``Why bundle proofs with data at all?'' discussion in \hyperref[sec:06-groups:06-why-bundle:1]{Chapter 6, Section 6} is the payoff this definitional transparency is building toward.
\end{quote}
