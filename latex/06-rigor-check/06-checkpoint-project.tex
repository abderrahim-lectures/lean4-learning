\begin{pblproject}
\label{sec:06-rigor-check:06-checkpoint-project:1}
\textbf{\large Checkpoint project: a Monoid from scratch}\par\smallskip


Part I is now complete, terms and types, structures, propositions and proofs, tactics, and the rigor questions just answered. Before Chapter 7 builds \texttt{Group}, this project asks for a smaller, self-contained version of exactly that construction, a \textbf{monoid}, a set with an associative operation and a two-sided identity, but \emph{no} inverses. Everything needed is already in hand from Chapters 1--6 alone; nothing here depends on \texttt{Group}, so it previews the construction of the next chapter rather than spoiling it.

\textbf{Learning objectives.} Practice bundling data with proof obligations in a \texttt{structure} (Chapter 3), writing tactic-mode proofs for a genuine algebraic axiom set (Chapter 5), and building a small ``prove it once, generically'' theorem in the style Chapter 8 will use for \texttt{Group}, here done for the weaker \texttt{Monoid} first.

\textbf{Prerequisites.} Chapters 1--6 only.
...
\tightlist
\item
  Define \texttt{structure Monoid (M : Type) where} with fields \texttt{op}, \texttt{id}, and proof obligations \texttt{assoc}, \texttt{id\_\allowbreak{}left}, \texttt{id\_\allowbreak{}right}, exactly the fields of \texttt{Group} (Chapter 7, Section 2), minus \texttt{inv}/\texttt{inv\_\allowbreak{}left}/\texttt{inv\_\allowbreak{}right}.
\item
...
\item
  Prove a generic theorem for an arbitrary \texttt{Mn : Monoid M}: \emph{the identity is unique}, if \texttt{e' : M} satisfies \texttt{∀ a, Mn.op e' a = a}, then \texttt{e' = Mn.id}. This is exactly Theorem 1 of Chapter 8 (\texttt{id\_\allowbreak{}unique}) carried out one chapter early, and, worth checking directly, its proof never needs an inverse, so nothing about lacking one gets in the way.
\end{enumerate}
...
If this compiles (\texttt{lake env lean} on a file containing it, or pasted into \texttt{lean\_\allowbreak{}project}), the project is done. A full worked solution, including a second instance, is in \hyperref[sec:15-appendix-solutions:06-chapter-6:1]{Appendix, Chapter 6}.
