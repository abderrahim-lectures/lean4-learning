...
\lstdefinestyle{lean}{
  ...
  % `columns=flexible` silently dropped the literal space immediately
  % following any newunicodechar-substituted symbol (confirmed by
  % isolating this in a minimal test file: "M -> M" rendered as "M ->M"
  % regardless of how the substitution itself was written, but rendered
  % correctly under `columns=fixed`) -- every arrow/logic/relation symbol
  % in this book's Lean code was silently missing its trailing space.
  columns=fixed,
  % Even under columns=fixed, `listings` scrambles character order and
  % swallows surrounding spaces around raw non-ASCII symbols (e.g. `{α`
  % renders as `α{`, `α → Nat` renders as `α→  Nat`) -- a listings/XeTeX
  % column-bookkeeping bug, not a newunicodechar artifact. build_latex.py's
  % escape_lstlisting_unicode() routes every such symbol through this
  % escape instead of leaving it as a raw character for listings to
  % measure and position itself.
  escapeinside={(*}{*)},
  frame=single,
  framerule=0.3pt,
