...
exactly the habit of a mathematician saying "it suffices to show...".
...
2. **Look for structure to induct or case-split on.** Both sides mention
   ...
   induct on. Trying [`induction b`](https://lean-lang.org/doc/reference/latest/Tactic-Proofs/Tactic-Reference/) is suggested by the recursion on the
   second argument of `+`, since `b` is the one that unfolds cleanly.
3. **Read the two resulting goals.** After `induction b with | zero => _ | succ k ih => _`
   ...
   ![The Lean Infoview panel in VS Code, showing the tactic state (hypotheses `a k : Nat`, `ih : a + k = k + a`, goal `a + (k + 1) = k + 1 + a`) for the succ case of the induction in my_add_comm.](images/goal-state-infoview.png)
4. **Find the missing lemmas.** `Nat.add_succ` and `Nat.zero_add` need not
   ...
   [`exact?`](https://lean-lang.org/doc/reference/latest/Tactic-Proofs/Tactic-Reference/) (Chapter 13) searches the whole environment for something that
