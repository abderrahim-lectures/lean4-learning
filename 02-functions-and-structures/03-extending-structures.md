...
- Mac Lane ([MacLane1998]), Ch. I §3 "Functors," p. 14 — the categorical reading of the `.toPoint`/`.toGroup` projection generated by `extends`, verified verbatim: "A functor which simply 'forgets' some or all of the structure of an algebraic object is commonly called a forgetful functor (or, an underlying functor). Thus the forgetful functor $U : \mathbf{Grp} \to \mathbf{Set}$ assigns to each group $G$ the set $UG$ of its elements..."
