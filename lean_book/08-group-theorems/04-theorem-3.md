...
Theorem 2 already establishes that *to show `x = Grp.inv y`, it suffices to
...
via a [universal property](../02-terminology-and-coc/01-terminology.md#category-theory-terms-used-beyond-the-baseline)
rather than by direct computation.

### The payoff, made concrete: applying `inv_op` to a real group

Chapter 7 promised that a theorem proved once, generically, is available
...
immediately to `perm3Group` (the non-abelian permutation group of Chapter 7),
...
Both sides agree on every input, exactly what the proof of `inv_op`
...
`perm3Group` is non-abelian required revisiting the proof of `inv_op` at all.
