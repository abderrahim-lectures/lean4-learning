...
(More scalable representations exist, `Fin 2 → Fin 2 → Int`, or the general
[`Matrix`](https://loogle.lean-lang.org/?q=Matrix) of Mathlib, but four named fields keep every computation
...
**Mathlib equivalent.** `Matrix (Fin 2) (Fin 2) Int` in Mathlib is exactly
...
Like Clarissa at her clarinet [Coyle2009, Ch. 2], the proof below reaches
to the edge of what has been built so far. The first axiom, `mul_assoc`,
is the hardest: it establishes the pattern. The remaining axioms repeat
that pattern against the same four cross terms, entry by entry.
...
1. **Every proof obligation here is spelled out explicitly, with no
   ...
   commutative-ring identities, is not actually available. Each entry
   equation is instead unfolded by hand through
   ...
   to match them. This is the deep practice of the chapter: `mul_assoc`
   is the hardest instance and teaches the pattern; `one_mul`, `mul_one`,
   `left_distrib`, and `right_distrib` are the same reach repeated against
   the same four cross terms. The `Ring Mat2` bundle itself is still noncommutative;
...
2. **This is the general pattern for "ring of $n\times n$ matrices over a
   ...
   the theorems of Chapter 10, the `Ring` hierarchy of Mathlib, has to handle
...
section deriving all five ring axioms for `Mat2` by hand (the `add4_reorder`
helper, reused twelve times across those axioms), Mathlib already proves
...
And where `mat2Ring` in the book needs a custom `Int`-arithmetic rewrite
...
of the `ring` tactic that the note above states is not available in this
book without Mathlib.
