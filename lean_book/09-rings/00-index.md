...
1. **What, precisely, changes when a second operation is layered onto a
   ...
   (R1)–(R4), the way Chapter 7 opened with the group axioms before any
   Lean.
2. **Axiom (R1) demands *commutative* addition, but `Group` in Chapter 7
...
5. **`intRing` is commutative and infinite. Does every ring axiom have to
   ...
   finite. The axioms of $\mathbb{Z}/3\mathbb{Z}$ are decidable statements
   about only finitely many elements, so `decide` can check them by brute
   enumeration instead.
6. **How is the data of this structure actually used, now that it nests a
   whole `CommGroup` inside it?** ([Section 6](06-accessing-fields.md))
   By ordinary field projection, chained one level deeper than Chapter 7,
...
end of Chapter 7: a precise definition, both a commutative and a
noncommutative witness that it is satisfiable, and, as Chapter 10 takes
