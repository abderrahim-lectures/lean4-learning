...
checked here. `fin3Ring` is the quotient (the ring $\mathbb{Z}/3\mathbb{Z}$ obtained by identifying elements that differ by a multiple of 3) $\mathbb{Z}/(3)$ by the ideal (the set of all multiples of 3) generated by $3$, with the built-in wraparound arithmetic of `Fin 3` giving
