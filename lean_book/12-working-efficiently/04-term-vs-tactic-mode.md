...
Every tactic-mode proof compiles down to a term (the style of Chapter 3). The
...
- **Term mode** is preferable for short proofs that are naturally a single
  ...
  one-line group-axiom proofs of Chapter 6 are a good example of where tactic mode
...
- **Tactic mode** is preferable once a proof involves several steps in a
  ...
  [`rw`](https://lean-lang.org/doc/reference/latest/Tactic-Proofs/Tactic-Reference/) chains of Chapters 7 and 9 would be hard to read (and much harder to *write*) as raw
  terms.
