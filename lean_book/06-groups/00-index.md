...
1. **A monoid has an associative operation and an identity — what is
   ...
   invertible. Adding it to the monoid's data turns "a set with an
...
4. **Does that abelian example secretly rely on commutativity somewhere
   `Group`'s definition does not require it to?** ([Section 4](04-permutations-example.md))
...
5. **Now that two concrete groups exist, how is their data actually used?**
   ([Section 5](05-accessing-fields.md)) The same way any structure's
   fields are used, from Chapter 2 onward: ordinary projection, whether
