...
First, we need the $\mathbb{Z}$-module structure on $\mathbb{Z}$ itself.
We have only described it in passing so far:
...
*generically*, using [`Submodule.span`](https://loogle.lean-lang.org/?q=Submodule.span). So `evenSubmodule`'s three
