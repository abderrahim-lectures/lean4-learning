...
(`Group`), Chapter 8 (`Ring`). `MyGroup` in the Chapter 5 appendix (exercise
...
2. Register at least two instances per class (e.g. `Int` and `Bool`,
   reusing the proofs of `intGroup`/`boolXorGroup` from Chapters 6–8).
...
data of a `Ring`, and connecting the `Module` vocabulary of Chapter 10 forward
to `Quiver`/`Path` of Chapter 11.

**Prerequisites.** Chapter 8 (`Ring`), Chapter 10 (`Module`), Chapter 11
(`Path`), the checkpoint project of Chapter 11 (`Path.length`) as a warm-up, and
the sketch of `PathAlgebraElem` in Exercise 3 of Chapter 11.
...
4. Assemble `Ring (PathAlgebraElem exampleQuiver k)` for a small, fully
   concrete `k` (`fin3Ring`, `Fin 3` from Chapter 8, Section 5, is a good first
...
`fin3Ring` in Chapter 8, Section 5.

### 3. Acyclic quivers have finite-dimensional path algebras

**Learning objectives.** Combinatorial reasoning about `Path.length`
(the checkpoint project of Chapter 11), and connecting it to the `Module`
notion of a finite spanning set from Chapter 10.
...
4. Conclude finite-dimensionality: an acyclic quiver on finitely many
   ...
   finite-dimensional `Module` over `k` in the sense of Chapter 10.
...
**Prerequisites.** Chapter 11 (the `Quiver`/`Path` of this book), Project 1
...
1. Read the `Quiver` class of `Mathlib.Combinatorics.Quiver.Basic` (already
   introduced in the "Mathlib equivalent" box of Chapter 11, Section 3) and
...
2. Find `Prefunctor` in Mathlib (a quiver homomorphism) and, separately,
   ...
   (`exampleQuiver`, `cyclicQuiver` from the exercises of Chapter 11),
   mapping vertices to vertices and arrows to paths.
3. Write the identity `QuiverHom` and composition of two `QuiverHom`s,
   noticing the parallel to the `LinearMap` exercises of Chapter 10 (identity
...
**Learning objectives.** Turn the closing remark of Chapter 11, Section 5,
...
1. Using the example quiver and the finite path algebra of Project 2, assign a
   ...
   `fin3Ring`-flavored addition of Chapter 8, Section 5) and a linear map to each arrow, exactly
   the data of a quiver representation (the remark of Chapter 11, Section 5, made
   concrete).
2. Package this data as a genuine `Module (PathAlgebraElem exampleQuiver
   k) (SomeCarrier)` instance (the `Module` structure of Chapter 10, Section 2), using
