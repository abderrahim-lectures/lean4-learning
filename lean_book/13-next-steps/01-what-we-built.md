...
used sparingly outside the discussion of it in Chapter 12, and only when a
genuine technical obstacle makes the explicit alternative not worth the
detour. `Perm3.ext` in Chapter 6 reaches for `simp only [mk.injEq]` because
core Lean generates no field-wise extensionality lemma for a plain
`structure`, and the checkpoint project of Chapter 11 reaches for
