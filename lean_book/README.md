...
All the PDF-build tooling lives in [`build/`](build/): `python
build/build_pdf.py` concatenates every chapter in reading order and
builds a single, print-style PDF (`lean-for-working-algebraists.pdf`, at
the book root) via Pandoc and XeLaTeX (KOMA-Script `scrbook`, with a
title page naming the actual author, an "About this book" page
disclosing that the text was generated by Claude Code under that
author's direction, numbered chapters/sections, and a table of
contents). It needs:

- `pandoc` and a working XeLaTeX install (e.g. MiKTeX or TeX Live). The
  build uses the free, cross-platform **TeX Gyre Pagella** (a Palatino
  equivalent, with the matching TeX Gyre Pagella Math for math) and
  **DejaVu Sans Mono** for code, so no proprietary fonts are required —
  install them via your TeX distribution's font packages (e.g.
  `texlive-fonts-extra` and `fonts-texgyre` plus `fonts-dejavu` on
  Debian/Ubuntu) if XeLaTeX cannot find them. Swap the `-V mainfont`/
  `-V mathfont` flags in `build/build_pdf.py` and `\setmonofont{...}` in
  `build/pdf-header.tex` for different fonts if preferred.
- `npx`/Node.js, to render every ```mermaid diagram to a real image via
  `@mermaid-js/mermaid-cli` (cached by content hash in
  `build/_mermaid_cache/`; falls back to the diagram's source text if
  `npx` isn't on `PATH`, same as in a non-Mermaid Markdown viewer).

Lean code blocks are syntax-highlighted using a custom Kate definition
(`build/lean4.xml` — Pandoc's highlighter has no built-in Lean lexer);
Python code blocks highlight out of the box. Long code lines wrap instead
of overflowing the page margin. Per-file navigation strips and
cross-file links don't make sense in one linear PDF, so the script
strips the former and flattens the latter to plain text. Released builds
of this PDF are attached to [GitHub releases](../../releases).
