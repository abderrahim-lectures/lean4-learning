...
- `pandoc` and a working XeLaTeX install (e.g. MiKTeX or TeX Live), with
  the fonts Palatino Linotype, Consolas, and Cambria Math available — all
  three ship with Windows/Office; substitute equivalents in
  `build/pdf-header.tex` and the `-V mainfont`/`-V mathfont` flags in
  `build/build_pdf.py` on other platforms.
