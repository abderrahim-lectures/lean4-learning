...
1. **What, precisely, changes when a second operation is layered onto a
   ...
   Lean.
2. **Axiom (R1) demands *commutative* addition, but `Group` in Chapter 6
...
4. **Is the definition non-vacuous?** ([Section 4](04-integers-example.md))
   Yes: $(\mathbb{Z}, +, \times, 0, 1)$, reusing `intGroup` from Chapter 6
...
7. **`intRing` is commutative. Does anything in the definition of `Ring`
   actually forbid that, the way permutations in Chapter 6 tested `Group`
