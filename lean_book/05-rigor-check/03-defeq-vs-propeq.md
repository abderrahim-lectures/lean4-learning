...
common source of real confusion once proofs get more intricate. It's
worth separating them out clearly now.

### Definitional equality

Two terms are **definitionally equal** ($a \equiv b$, sometimes written
$a \equiv_\beta b$ to stress that it's driven by reduction) if they reduce to
...
check that two types or terms match) or it doesn't, and `rfl` is the tactic
...
One precise point worth having: "reduce to the same normal form" doesn't mean
...
prove any other statement. Definitional equality is only the easiest
...
proof component doesn't matter; only the data (`op`, `id`, `inv`) can
actually differ.

### A note on structure eta

This is a companion fact, relied on silently whenever this book (or you) writes
