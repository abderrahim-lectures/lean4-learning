...
Chapters 1–4 have relied on the type checking of Lean constantly the same
...
extra generality is exactly the Π-types of Chapter 1, Section 5, already covered.
...
goal state since Chapter 4. The rules that generate valid judgments are as follows.
...
- **(Abs)**: to type-check `fun x => t`, extend the context with
  ...
  [`intro`](https://lean-lang.org/doc/reference/latest/Tactic-Proofs/Tactic-Reference/) in Chapter 4), check that the type of `t` is `τ2` under that extended
...
- **(App)**: to type-check `t1 t2`, `t1` must have a function type whose
  ...
  This is exactly the error discussed in Chapter 4 under "reading a tactic
...
This is precisely the gap already closed by [Chapter 1, Section 5](../01-basics/05-pi-sigma-and-coc.md).
...
formal system CoC/CIC named by Chapter 1, Section 5, extending STLC above with
...
`Sort 0` is `Prop` and `Sort (k+1)` is `Type k` (Chapter 1, Section 5).
...
- Girard: see [the references of Chapter 5, Section 2](02-universes.md) for the full citation. "Girard's paradox" (the inconsistency of `Type : Type`) is due to the 1972 thesis of Girard, a different, later paper than [Girard1971] cited elsewhere in this book.
