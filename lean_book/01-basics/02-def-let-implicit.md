...
- `def` — the keyword that introduces a new global definition into the
  ...
  once elaborated, `double` is a completely ordinary constant you can refer
  to anywhere later in the file (or, once the file is imported, anywhere
  else). Contrast with `example` from Chapter 3, which elaborates a term
...
- `(n : Nat)` — an explicit argument named `n`, of type `Nat`. "Explicit"
  ...
  *actual* type ends up being `Nat → Nat`, exactly as if you'd written
  `def double : Nat → Nat := fun n => n * 2` instead. The two forms
  elaborate to the same term; the `(n : Nat)` binder form is just the
  standard, more readable surface syntax for "a function with named
  parameters."
- `: Nat` (the second one, right before `:=`) — the declared return type.
  This isn't optional filler: Lean uses it to *check* the body against a
  known expected type while elaborating, rather than only inferring a type
  afterward. If the body's type didn't match, you'd get an error at this
  point, not somewhere downstream.
...
- `n * 2` — the body. At this point in elaboration, `n` is in scope with
  ...
  `Nat` multiplication applied to `n` and the numeral `2`, which itself
  elaborates to a `Nat` because that's what `Nat.mul`'s second argument's
  type forces it to be — numeral elaboration is guided by expected type,
  another instance of Lean checking against context rather than guessing.
...
$\mathrm{double}(n) = 2n$ becoming `:= n * 2`. There is no semantic
difference between writing the domain as one arrow `Nat → Nat` or as a
...
- `(a b : Nat)` — two explicit arguments, both of type `Nat`, written with
  ...
  `(a : Nat) (b : Nat)`; Lean expands it identically either way. Use this
  shorthand whenever several consecutive parameters share a type — it's
  standard style, not a shortcut that costs you anything.
- `let sum := a + b` — introduces a *local* definition, visible only in the
  ...
  `a + b` is already fully determined by `a` and `b`'s types — Lean infers
  it. Operationally, `let sum := a + b; sum / 2` means exactly the same
  thing as substituting `a + b` for every occurrence of `sum` in `sum / 2`
  — a `let` is definitionally transparent, so `average a b` and
  ...
  means, and in a longer proof or definition, prevents you from repeating
  a nontrivial subexpression (and therefore repeating a mistake in it) in
  several places.
...
with the caveat that this `average` computes $\lfloor s/2 \rfloor$
(floor division) rather than the true rational average $s/2$, since
`Nat`'s division is truncating — a discrepancy worth flagging exactly
...
- `{α : Type}` — an **implicit** argument, marked by curly braces instead
  ...
  variable — again pure convention, `T` or `A` would work identically) has
...
- The crucial difference from `(n : Nat)` above: `{α : Type}` is not
  ...
  unification, working backward from the type of the explicit argument you
  did supply. In `identity 5`, Lean sees you're passing `5 : Nat` where an
  `x : α` is expected, unifies `α := Nat`, and only then checks the rest.
  You *can* still supply an implicit argument explicitly if you ever need
  to override inference, with `@identity Nat 5` — the `@` prefix means "no
  more auto-inference, I'm giving every argument, implicit or not, by
  hand." This escape hatch matters mainly for debugging elaboration
  failures, not everyday use.
- Why implicit rather than explicit here at all: `α` is determined by
  `x`'s type at every call site, so making the caller type it out (as in
  `identity Nat 5`) would be pure noise — Lean already has enough
...
- `: α := x` — the return type is `α` itself (the same type variable bound
  ...
  per type, which is exactly what the `{α : Type}` parameter buys you.

**Mathematical reading.** `identity {α : Type} (x : α) : α := x` is the
family $\{\,\mathrm{id}_A : A \to A\,\}_{A \in \mathbf{Type}}$ indexed over
*every* type $A$ at once — precisely the assignment sending each object
$A$ of the category to its identity morphism $\mathrm{id}_A$, packaged as a
single polymorphic definition rather than one definition per $A$. The
implicit argument `{α : Type}` is what makes this a *statement about all
$A$ simultaneously* rather than a single fixed function; in category
theory you'd never write "$\mathrm{id}_{\mathbb{Z}}$, $\mathrm{id}_{\mathbb{R}}$, ..."
one at a time either — you'd say "for every object $A$, there is an
identity morphism $\mathrm{id}_A$," which is exactly what the
universally-quantified, implicitly-inferred `{α : Type}` expresses.
