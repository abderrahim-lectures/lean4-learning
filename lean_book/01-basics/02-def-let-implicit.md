...
Here are three short definitions. Every token in them is doing something
specific. Let's go through each one in full.
...
- `def` — the keyword that introduces a new global definition into the
  ...
  term but does *not* bind it to a name. `def` is for the "I want to reuse
  this" case.
...
- `n * 2` — the body. At this point in elaboration, `n` is in scope with
  ...
  itself elaborates to a `Nat`, because that's what `Nat.mul`'s second
...
- `(a b : Nat)` — two explicit arguments, both of type `Nat`, written with
  ...
  standard style, and it costs you nothing.
- `let sum := a + b` — introduces a *local* definition, visible only in the
  ...
  means, and in a longer proof or definition, it stops you from repeating
...
- `sum / 2` — `Nat` division, which in Lean is *truncating*. `average 4 10`
  ...
  fractions). Keep this in mind before relying on this `average` for
...
truncating. This gap is worth flagging exactly because it's easy to skip
over in ordinary mathematical prose, but Lean forces you to confront it —
there is no coercion to $\mathbb{Q}$ happening for free.
...
category theory you'd never write "$\mathrm{id}_{\mathbb{Z}}$,
$\mathrm{id}_{\mathbb{R}}$, ..." one at a time either. You'd say "for every
object $A$, there is an identity morphism $\mathrm{id}_A$," which is
