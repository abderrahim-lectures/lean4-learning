...
the logic of Chapter 3, then real proofs, loose use of these words would
...
formal treatment lives. [Chapter 1, Section 5](05-pi-sigma-and-coc.md) covers
Π/Σ-types and the calculus of constructions, [Chapter 3,
Section 2](../03-propositions-and-proofs/02-logic-recap.md) covers the logic
underneath Curry–Howard, and [Chapter 5,
Section 3](../05-rigor-check/03-typing-rules-and-safety.md) covers typing rules and
...
(Chapter 1), the subgoal-matching done by `apply` (Chapter 4), and typeclass
...
(Chapter 1) computes the normal form of a term and prints it. `rfl` (Chapter 3)
...
Church-encoding aside of Chapter 13).
...
`Path` type of Chapter 11, whose very type depends on specific vertices) rather than in a
...
property (see [Chapter 1, Section 1](01-everything-has-a-type.md) for the
worked-out $F$-algebra/NNO discussion this box is summarizing).
`ℤ` in `Ring` (Chapter 8) *is* an honest initial object of the category of
