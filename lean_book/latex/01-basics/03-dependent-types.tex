...
\texttt{Vec.replicate} recurses exactly once per unit of length, so it is worth watching the recursion unwind one call at a time. Adding a \texttt{dbg\_\allowbreak{}trace} line to each branch (harmless --- it only prints, and changes nothing about what the function returns) makes every step visible:
