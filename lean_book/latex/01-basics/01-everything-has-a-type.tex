...
In this language, \texttt{Nat} is the \href{https://ncatlab.org/nlab/show/initial+algebra+of+an+endofunctor}{\textbf{initial algebra}} (\cite{NLabInitialAlgebra}) for the \textbf{endofunctor} \(F(X) = 1 + X\) on \texttt{Type} (\texttt{1} a one-element type, Lean's \href{https://leanprover-community.github.io/mathlib4_docs/Init/Prelude.html\#Unit}{\texttt{Unit}}; \texttt{+} disjoint sum, Lean's \href{https://leanprover-community.github.io/mathlib4_docs/Init/Core.html\#Sum}{\texttt{Sum}}). An \(F\)-algebra is a type \(X\) together with a map \(F(X) \to X\), i.e.~a map \((1 + X) \to X\) --- equivalently, by the universal property of \texttt{+}, a chosen element \(e : X\) (the image of the \texttt{1} summand; any element --- nothing ties it to \texttt{Nat}'s own \texttt{zero}) together with a chosen self-map \(s : X \to X\) (the image of the \(X\) summand; any self-map --- nothing ties it to \texttt{succ}). A morphism of \(F\)-algebras from \((X, e, s)\) to \((Y, e', s')\) is a function \(f : X \to Y\) satisfying \(f(e) = e'\) and \(f(s(x)) = s'(f(x))\) for every \(x\). \texttt{Nat}, with \texttt{zero} and \texttt{succ} playing the roles of \(e\) and \(s\), is the \href{https://ncatlab.org/nlab/show/initial+object}{\textbf{initial object}} (\cite{NLabInitial}) among \(F\)-algebras: for every other algebra \((X, e, s)\), exactly one algebra morphism out of \texttt{Nat} exists, forced by sending \texttt{zero} to \(e\) and \texttt{succ n} to \(s\) applied to wherever \texttt{n} was sent. That forced uniqueness is exactly the universal property that makes structural induction valid: to define a function, or prove a statement, for every \texttt{Nat}, it suffices to say what happens at \texttt{zero} and how it is preserved by \texttt{succ}. This construction has a standard name: \texttt{Nat}, viewed this way, is a \href{https://ncatlab.org/nlab/show/natural+numbers+object}{\textbf{natural numbers object}} (\cite{NLabNNO}) of \texttt{Type} --- equivalently, the initial algebra for the endofunctor \(1 + (-)\).
...
\item
  nLab, ``initial algebra of an endofunctor'' (\cite{NLabInitialAlgebra}) --- the precise categorical framing used in the optional box's second paragraph: \texttt{Nat} as the initial algebra for \(F(X) = 1 + X\).
\item
  nLab, ``natural numbers object'' (\cite{NLabNNO}) --- the standard name for the same construction, equivalent to the initial-algebra framing above.
\end{itemize}
