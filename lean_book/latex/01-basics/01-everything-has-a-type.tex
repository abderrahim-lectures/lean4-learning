...
Nothing stops Python from \emph{writing} \texttt{add\_them(3, "oops")}. The mistake is only discovered the moment that specific line executes --- if it sits on a rarely-hit branch, it can ship for months undetected. And in the \texttt{True} case, Python does not even complain: it quietly coerces the boolean to \texttt{1} and moves on, whether or not that was the intended meaning.
