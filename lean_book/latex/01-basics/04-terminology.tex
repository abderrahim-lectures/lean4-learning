...
\subsection{Recall}\label{sec:01-basics:04-terminology:2}
...
\item
  \textbf{Subobject.} ``Let \(A\) be any category. If \(u : s \to a\) and \(v : t \to a\) are two monics {[}in \(A\){]} with a common codomain \(a\), write \(u \sim v\) when \(u\) factors through \(v\) \ldots{} the corresponding equivalence classes of these monics are called the subobjects of \(a\)'' (\cite{MacLane1998}, Ch. III §7, p.~126; equivalently \cite{Pareigis1970}, §1.6, p.~20). Brief: a piece of an object cut out by a condition, remembered together with its inclusion.
\item
...
\subsection{Elaborate / elaboration}\label{sec:01-basics:04-terminology:3}
...
\subsection{Unify / unification}\label{sec:01-basics:04-terminology:4}
...
\subsection{Reduce / reduction, normal form}\label{sec:01-basics:04-terminology:5}
...
Python's own \texttt{lambda} really does \textgreater{} β-reduce exactly like the calculus above on simple examples --- \textgreater{} \texttt{(lambda x: x + 1)(5)} reduces to \texttt{5 + 1} to \texttt{6}, the same substitution \textgreater{} step as \((\lambda x.\, x + 1)\, 5 \to_\beta 5 + 1\) --- but Python's \textgreater{} \texttt{lambda} is a deliberately limited subset: its body must be a single \textgreater{} \emph{expression}, no \texttt{if}/\texttt{for}/multiple statements. The actual untyped \textgreater{} λ-calculus has no such restriction, because none is needed: conditionals \textgreater{} and recursion are just more terms built from abstraction and \textgreater{} application, not separate features bolted on top. Lean's \texttt{fun} matches \textgreater{} the unrestricted calculus, not Python's narrower \texttt{lambda}.
...
\subsection{Motive}\label{sec:01-basics:04-terminology:6}
...
\subsection{Category-theory terms used beyond the baseline}\label{sec:01-basics:04-terminology:7}
...
\subsubsection{Universal property}\label{sec:01-basics:04-terminology:8}
...
\subsubsection{Initial object}\label{sec:01-basics:04-terminology:9}
...
\subsubsection{Forgetful functor}\label{sec:01-basics:04-terminology:10}
...
\subsubsection{Subobject / full subcategory}\label{sec:01-basics:04-terminology:11}
...
\subsection{References}\label{sec:01-basics:04-terminology:12}
