...
\subsection{Recall}\label{sec:10-modules:02-translating-into-lean:0b}
...
\tightlist
\item
  \textbf{Module.} ``A binary operation \(+\) on \(M\) under which \(M\) is an abelian group, and ... an action of \(R\) on \(M\) ... (a) \((r+s)m = rm+sm\) ... (b) \((rs)m = r(sm)\) ... (c) \(r(m+n) = rm+rn\) ... (d) \(1m = m\)'' (\cite{DummitFoote2003}, §10.1 ``Basic Definitions and Examples,'' p.~336).
\end{itemize}
...
\tightlist
\item
  Dummit and Foote (\cite{DummitFoote2003}), \S10.1 ``Basic Definitions and Examples,'' p.~336 --- module axioms (M1)--(M4).
\item
