...
Five lines print, one per \texttt{succ} layer stripped off \texttt{4}, each announcing ``one more copy of \texttt{m}'' \emph{before} the recursive call --- the same five-step shape \texttt{double}'s \texttt{Nat.rec} trace had in Chapter 1, since both recurse structurally on the same \texttt{Nat} argument. The final \texttt{12} is exactly \(3+3+3+3\), \texttt{intGroup}'s \texttt{op} (ordinary \texttt{Int} addition) applied four times. \texttt{intSmul} extends it to negative integers using the group inverse. Given any \texttt{CG : CommGroup M}, one can then check that \texttt{intSmul CG} satisfies (M1)--(M4) against \texttt{intRing} (Chapter 8). This is a genuine, if somewhat long, proof by induction on the integer scalar, in the same spirit as Chapter 4's \texttt{my\_\allowbreak{}add\_\allowbreak{}comm}. The full verification is left as an extended exercise, since carrying it out directly is the best way to see \emph{why} modules over \(\mathbb{Z}\) are forced, not chosen: \texttt{intSmul} is the \emph{unique} action that turns any abelian group into a \(\mathbb{Z}\)-module, because (M4) plus (M1)/(M2) pin down \(n \cdot m\) for every integer \(n\) by induction, leaving no freedom.
