...
\texttt{map\_add}'s goal, \texttt{d * (m+n) = d*m + d*n}, is exactly distributivity. \texttt{map\_smul}'s goal, \texttt{d * (r*m) = r * (d*m)}, holds because \(\mathbb{Z}\) is commutative, so \texttt{d} and \texttt{r} can swap past each other. Both fields, in other words, are pure \texttt{Int}-arithmetic facts once \texttt{toFun}, \texttt{ModM.smul}, and \texttt{ModM.addGrp.op} are unfolded to their meanings here. This is the same ``reduce a module-theoretic goal to a concrete arithmetic identity'' move that §4's \texttt{evenSubmodule} used, applied again.
...
This has the same shape as \texttt{mulByLinearMap}: a \texttt{toFun} and two proof obligations, with \texttt{map\_add}/\texttt{map\_smul} becoming Mathlib's \texttt{map\_add'}/\texttt{map\_smul'}. The real difference is that \texttt{Int →ₗ[Int] Int} is a type the rest of Mathlib already knows how to compose, transport along isomorphisms, and package into matrices. \texttt{LinearMap intRing intZModule intZModule} gets none of that for free.
