...
The single biggest efficiency gain, greater than any tactic choice, is to \textbf{prove the general fact once, as its own named lemma, as soon as an argument would otherwise be repeated.} Chapter 7's \texttt{left\textbackslash{}\_inverse\textbackslash{}\_unique} is the running example. Theorem 3 (\texttt{inv\textbackslash{}\_op}) and Chapter 9's \texttt{neg\textbackslash{}\_one\textbackslash{}\_mul} and \texttt{neg\textbackslash{}\_mul} all reduce to it, instead of re-deriving ``uniqueness of inverses'' inline. Signs that a lemma should be factored out:
...
\textbf{Key points.} \texttt{exact?}/\texttt{apply?} search for a closing term but do not always find the shortest one; \texttt{decide}/\texttt{omega}/\texttt{norm\textbackslash{}\_num} replace a hand proof exactly on their decidable fragment, never on a goal about a generic, unspecified structure; \texttt{simp} trades traceability for speed, so this book reaches for it only when a genuine technical obstacle makes the explicit alternative not worth the detour; and a repeated \texttt{rw} chain or an independently-statable sub-goal is a lemma waiting to be named.
