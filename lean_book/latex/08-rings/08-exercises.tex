...
\textbf{Key points.} A ring bundles an additive \texttt{CommGroup} with a multiplicative monoid and two distributive laws, \texttt{left\textbackslash{}\_distrib} and \texttt{right\textbackslash{}\_distrib} genuinely independent since \texttt{mul} need not commute. A finite carrier (\texttt{Fin 3}) lets every axiom be checked by \texttt{decide}; an infinite one (\texttt{Int}, matrices over \texttt{Int}) needs a real proof, or a computed counterexample (\texttt{\textbackslash{}\#eval}) to refute commutativity outright.
