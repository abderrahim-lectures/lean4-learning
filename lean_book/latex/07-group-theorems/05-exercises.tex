...
\tightlist
\item
  Prove \texttt{theorem inv\textbackslash{}\_inv (a : G) : Grp.inv (Grp.inv a) = a}. Before writing any tactics, consider: does this match the shape of a lemma already in hand (Theorem 2 again)? What single fact about \texttt{a} and \texttt{Grp.inv a} would permit invoking it directly?
\item
  Prove \texttt{theorem cancel\textbackslash{}\_left (a b c : G) (h : Grp.op a b = Grp.op a c) : b = c}. Strategy hint: \texttt{b} and \texttt{c} cannot be rewritten directly in isolation. Instead, apply \texttt{Grp.op (Grp.inv a)} to \emph{both sides} of \texttt{h} first (as a \texttt{have}), then simplify each side using \texttt{assoc}/\texttt{inv\textbackslash{}\_left}/\texttt{id\textbackslash{}\_left}, the same ``regroup, then cancel'' pattern as Theorem 3.
\end{enumerate}
