...
\textbf{Finding the proof.} This has the same shape as Theorem 1: an equality between two elements known only through separate facts (\texttt{h} about \texttt{b}, the \texttt{Group} axioms about \texttt{Grp.inv a}). Hence the same strategy applies: relate both sides to something in common. Here, however, no single lemma hands us \texttt{Grp.op b Grp.id} the way \texttt{Grp.id\_right} did in Theorem 1's simpler setting. We must \emph{build} the chain by rewriting \texttt{b} itself, step by step.
...
Each \texttt{=} above is licensed by exactly one \texttt{Group} field or the hypothesis \texttt{h}, in this order: \texttt{id\_right} (backwards), \texttt{inv\_right} (backwards), \texttt{assoc} (backwards), \texttt{h}, \texttt{id\_left}. Writing the \emph{paper} proof first, as a chain of equalities, and then reading off which axiom licenses each step, is often faster than guessing tactics directly against the goal. This chain is best worked out on scratch paper before the editor is opened.
...
Reading right to left through the \texttt{rw} list gives line-for-line the displayed chain above: \texttt{←} \href{https://loogle.lean-lang.org/?q=mul_one}{\texttt{mul\_one}} \texttt{b} turns \texttt{b} into \texttt{b * 1}; \texttt{←} \href{https://loogle.lean-lang.org/?q=mul_inv_cancel}{\texttt{mul\_inv\_cancel}} \texttt{a} turns that \texttt{1} into \texttt{a * a⁻¹}; \texttt{←} \href{https://loogle.lean-lang.org/?q=mul_assoc}{\texttt{mul\_assoc}} reassociates; \texttt{h} substitutes \texttt{b * a} for \texttt{1}; \href{https://loogle.lean-lang.org/?q=one_mul}{\texttt{one\_mul}} clears the resulting \texttt{1 * a⁻¹}. These are the same five steps, in the same order --- only the names \texttt{Grp.assoc}/\texttt{Grp.inv\_right} and so on are replaced by Mathlib's generic \texttt{mul\_assoc}/\texttt{mul\_inv\_cancel} and so on.
