...
\tightlist
\item
  \texttt{∀ x : α, P x} is again just a (dependent) function type: given any \texttt{x}, produce a proof of \texttt{P x}. \texttt{all\_nats\_ge\_zero} is literally a function taking any \texttt{n} and returning a proof that \texttt{n ≥ 0} --- here, \texttt{Nat.zero\_le n} already proves exactly that for whichever \texttt{n} gets passed in.
\end{itemize}
...
\textbf{A second example: there is always a bigger prime.} \texttt{exists\_prime\_gt\_three} is one concrete instance of a classical fact: no matter which number is chosen, there is a prime bigger than it (here the chosen number is \texttt{3}, and \texttt{5} is a prime that beats it). It follows the exact same ``number, then why'' shape as before, but with two differences worth noting:

\begin{itemize}
\tightlist
\item
  The \emph{property} being witnessed, \texttt{p > 3 ∧ isPrime p}, is itself an \texttt{∧}. So the full picture is a witness (\texttt{5}) plus a \emph{pair} of facts about it (\texttt{5 > 3}, and \texttt{5} is prime), all packed into the outer \texttt{⟨\_, \_⟩}.
\item
  The proof is not \texttt{rfl}. \texttt{p > 3 ∧ isPrime p} does not reduce to a plain equality, so instead the second slot is by \href{https://lean-lang.org/doc/reference/latest/Tactic-Proofs/Tactic-Reference/}{\texttt{decide}} --- the same brute-force tactic from \hyperref[sec:03-propositions-and-proofs:05-and-or-not:1]{Chapter 3 §5}'s \texttt{not\_example}, which here checks \texttt{5 > 3} outright and tries every candidate divisor below \texttt{5} to confirm none of them divide it.
\end{itemize}
...
\] literally an \(\alpha\)-indexed product: a proof is a function assigning to each \(x\) a proof of \(P(x)\), so \texttt{all\_nats\_ge\_zero} is the map \(n \mapsto (0
