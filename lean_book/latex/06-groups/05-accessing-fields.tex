...
This is the same contrast as §3: the book's \texttt{intGroup.op}/\texttt{.id}/\texttt{.inv} are projections out of a bundle built by hand, while Mathlib's \texttt{+}/\texttt{0}/ \texttt{-} are notation that the typeclass system has already wired to the right instance. The underlying ``which \texttt{AddCommGroup} instance is this?'' bookkeeping remains invisible unless sought out (for example, with \texttt{\#print}).
