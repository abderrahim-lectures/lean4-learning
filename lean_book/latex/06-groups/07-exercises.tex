...
\item
  \emph{\texttt{intGroup} never tests the difference between \texttt{id\_left}/\texttt{id\_right} or \texttt{inv\_left}/\texttt{inv\_right}, since \texttt{Int} addition is commutative. Does that mean the split is only there for generality's sake, with no concrete example that actually needs it?} No --- \texttt{perm3Group} needs it for real: composing permutations is genuinely non-commutative, so \texttt{id\_left} and \texttt{id\_right} are two separate proof obligations there, not a single fact proved twice.
\item
