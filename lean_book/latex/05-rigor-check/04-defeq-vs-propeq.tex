...
\subsection{Recall}\label{sec:05-rigor-check:04-defeq-vs-propeq:0b}
...
\item
  \textbf{Propositional equality.} ``In other situations ... reductions hold only propositionally, which is to say, they are equational theorems that must be applied explicitly'' (\cite{TPIL4}, ``Dependent Types'' / ``Tactics''). Brief: an ordinary \texttt{Prop} that must be proved; definitional equality implies propositional equality, but not conversely.
\item
  \textbf{Proof irrelevance.} ``If \texttt{p : Prop} is any proposition, Lean's kernel treats any two elements \texttt{t1 t2 : p} as being definitionally equal ... This is known as proof irrelevance'' (\cite{TPIL4}, ``Propositions and Proofs'').
\end{itemize}
