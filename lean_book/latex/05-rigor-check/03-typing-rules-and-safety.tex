...
\subsection{Recall}\label{sec:05-rigor-check:03-typing-rules-and-safety:2}
...
\item
  \textbf{Universe-formation rule.} This book's working statement, after the calculus of constructions (\cite{CoquandHuet1988}): \(\mathtt{Type}\,i : \mathtt{Type}\,(i+1)\), and a Π-type built from \(A : \mathtt{Type}\,i\), \(B : \mathtt{Type}\,j\) lands in \(\mathtt{Type}\,(\max(i,j))\).
\end{itemize}

Chapters 1--4 have relied on Lean's type checking constantly without ever seeing its rules written down. This section makes two things precise: the actual rules Lean's kernel checks a term against (using a small, representative fragment, the \textbf{simply typed λ-calculus}, STLC), and the specific rule governing the universe hierarchy Section 2 just introduced informally.

An untyped calculus has a serious defect: it permits terms that reduce forever (Chapter 1, Section 4 mentioned β-reduction; nothing there stopped a term from β-reducing to itself, endlessly), and nothing prevents applying one kind of value where a completely different kind was intended. STLC fixes this by attaching a \textbf{type} to every term and requiring application to respect types --- the direct ancestor of Chapter 1's ``every expression has a type.''

\subsection{Types}\label{sec:05-rigor-check:03-typing-rules-and-safety:3}

\[
\tau ::= \iota \;\mid\; \tau \to \tau
\]

where \(\iota\) ranges over some fixed collection of \textbf{base types} (think \texttt{Nat}, \texttt{Bool}) and \(\tau_1 \to \tau_2\) is the type of functions from \(\tau_1\) to \(\tau_2\) --- exactly Lean's \texttt{→}. Every type is built from base types and arrows. There is not yet any way to quantify over \emph{all} types the way Chapter 1's \texttt{identity \{α : Type\} (x : α) : α := x} did --- that extra generality is exactly Chapter 1, Section 5's Π-types, already covered.
...
\subsection{Typing judgments and rules}\label{sec:05-rigor-check:03-typing-rules-and-safety:4}
...
\subsection{Type preservation and progress: the payoff}\label{sec:05-rigor-check:03-typing-rules-and-safety:5}
...
\item
  \textbf{Preservation} (subject reduction): if \(\Gamma \vdash t : \tau\) and \(t \longrightarrow_\beta t'\), then \(\Gamma \vdash t' : \tau\). Reduction never changes a term's type. This is \emph{exactly} why this chapter's own definitional equality (Section 4, next) is trustworthy: reducing a term to normal form (what \texttt{\#eval}/\texttt{rfl} do) can never accidentally produce something of a different type than the one it started with.
\end{itemize}
...
\subsection{What STLC still cannot do}\label{sec:05-rigor-check:03-typing-rules-and-safety:6}
...
This is exactly Section 2's \texttt{Group : Type → Type} computation spelled out as a general rule: with \(A = \mathtt{Type}\) (itself living in \texttt{Type 1}) and \(B = \mathtt{Type}\) again, the rule gives \(\max(1, 1) = 1\), so \texttt{Type → Type} lands in \texttt{Type 1}, one level above \texttt{Type} itself.
...
\subsection{References}\label{sec:05-rigor-check:03-typing-rules-and-safety:8}
...
\item
  Girard --- \textbf{Correction:} see the note in \hyperref[sec:05-rigor-check:02-universes:1]{Section 2's References} --- ``Girard's paradox'' (the inconsistency of \texttt{Type : Type}) is due to Girard's 1972 thesis, not the 1971 paper cited as \cite{Girard1971} elsewhere in this book.
\item
  \emph{Theorem Proving in Lean 4} (\cite{TPIL4}), §2.2 ``Types as objects'' --- Lean's own documentation on universes, matching the presentation here.
\end{itemize}
