...
\subsection{Recall}\label{sec:05-rigor-check:02-universes:0b}
...
\item
  \textbf{Universe polymorphism.} ``Some operations ... need to be polymorphic over type universes. For example, \texttt{List α} should make sense for any type \texttt{α}, no matter which type universe \texttt{α} lives in ... Lean allows you to declare universe variables explicitly using the \texttt{universe} command'' (\cite{TPIL4}, §2.2).
\end{itemize}
...
\subsection{References}\label{sec:05-rigor-check:02-universes:5}
