...
\tightlist
\item
  Define \texttt{structure Monoid (M : Type) where} with fields \texttt{op}, \texttt{id}, and proof obligations \texttt{assoc}, \texttt{id\_left}, \texttt{id\_right} --- exactly \texttt{Group}'s fields (Chapter 6 §2), minus \texttt{inv}/\texttt{inv\_left}/\texttt{inv\_right}.
\item
...
\item
  Prove a generic theorem for an arbitrary \texttt{Mn : Monoid M}: \emph{the identity is unique} --- if \texttt{e' : M} satisfies \texttt{∀ a, Mn.op e' a = a}, then \texttt{e' = Mn.id}. This is exactly Chapter 7's Theorem 1 (\texttt{id\_unique}) carried out one chapter early, and --- worth checking directly --- its proof never needs an inverse, so nothing about lacking one gets in the way.
\end{enumerate}
...
If this compiles (\texttt{lake env lean} on a file containing it, or pasted into \texttt{lean\_project}), the project is done. A full worked solution, including a second instance, is in \hyperref[sec:14-appendix-solutions:04-chapter-5:1]{Appendix, Chapter 5}.
