...
\item
  Pierce (\cite{Pierce2002}), §11.6 ``Pairs,'' §11.7 ``Tuples,'' §11.8 ``Records'' --- records and products as the standard language-theoretic account of what \texttt{structure} implements, verified verbatim: ``The simplest of these is pairs, or more generally tuples, of values\ldots{} {[}§11.8{]} the generalization from n-ary tuples to labeled records is equally straightforward.''
\item
  Bodo Pareigis, \emph{Categories and Functors} (\cite{Pareigis1970}), §1.11, p.~30 --- the universal-property definition of a categorical product used in the ``Mathematical reading'' box above, verified verbatim: ``there is exactly one morphism \(h : C \to A \times B\) such that \(f = p_A h\) and \(g = p_B h\).''
\end{itemize}
