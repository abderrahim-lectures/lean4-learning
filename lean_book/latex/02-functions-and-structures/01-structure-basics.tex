...
\tightlist
\item
  \texttt{Point.mk} is the automatically generated \textbf{constructor}. This is the function that actually builds a \texttt{Point} out of an \texttt{x} and a \texttt{y}. \texttt{\textbackslash{}\{ x := ..., y := ... \textbackslash{}\}} is shorthand for \texttt{Point.mk ... ...}. It names each field so the order cannot be mixed up.
\item
  There is an even shorter way to write this: \texttt{⟨0, 0⟩} builds the exact same \texttt{Point}. It simply lists the values in field-declaration order instead of naming them. Read \texttt{⟨\textbackslash{}\_, \textbackslash{}\_⟩} as \textbf{``here are the pieces, in order --- the constructor is to be inferred from context.''} Lean can always determine it, because the \emph{expected type} (here, \texttt{Point}, from \texttt{def origin : Point := ...}) indicates exactly which constructor and which fields must be filled in. This is also where the official name comes from: it is called the \textbf{anonymous constructor}, because \texttt{Point.mk} is never written explicitly --- it remains anonymous, and Lean infers it from context. \texttt{⟨\textbackslash{}\_, \textbackslash{}\_⟩} recurs constantly from Chapter 3 onward, for proofs as much as for data.
\item
...
\texttt{shift} shows a structure used on \emph{both} sides of a function: it takes a \texttt{Point} in (reading its fields back out with the same \texttt{p.x}/\texttt{p.y} projection notation) and builds a new one via \texttt{\textbackslash{}\{ x := ..., y := ... \textbackslash{}\}}, the same field-naming syntax \texttt{origin} used above.
