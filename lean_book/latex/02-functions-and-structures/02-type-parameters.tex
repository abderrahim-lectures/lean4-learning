...
\subsection{Recall}\label{sec:02-functions-and-structures:02-type-parameters:0b}
...
\tightlist
\item
  \textbf{Type-parameterized structure.} This book's working statement: \texttt{structure Pair (α β : Type)} defines a whole \emph{family} of products at once --- one instance per choice of type arguments --- categorically a functor of the type parameters, not a single fixed structure.
\item
  \textbf{Parametric polymorphism.} After Milner (\cite{Milner1978}), the theoretical account of type polymorphism that Python's \texttt{TypeVar} and Lean's \texttt{\{α : Type\} → ...} both implement to different degrees. Brief: genericity that is \emph{proved} once for every type, checked before the generic code is ever called, unlike an optional/erased type annotation.
\end{itemize}
