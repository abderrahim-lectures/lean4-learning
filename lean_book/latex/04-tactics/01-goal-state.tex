...
\item
  ...
  \begin{figure}
  \centering
  \pandocbounded{\includegraphics[keepaspectratio,alt={The Lean Infoview panel in VS Code, showing the tactic state (hypotheses a k : Nat, ih : a + k = k + a, goal a + (k + 1) = k + 1 + a) for the succ case of my\_add\_comm's induction.}]{../04-tactics/images/goal-state-infoview.png}}
  \caption{The Lean Infoview panel in VS Code, showing the tactic state (hypotheses \protect\texttt{a k : Nat}, \protect\texttt{ih : a + k = k + a}, goal \protect\texttt{a + (k + 1) = k + 1 + a}) for the \protect\texttt{succ} case of \protect\texttt{my\_\allowbreak{}add\_\allowbreak{}comm}'s induction.}
  \end{figure}
\item
