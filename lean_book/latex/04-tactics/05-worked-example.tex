...
\tightlist
\item
  The left side, \texttt{a + 0}, reduces to \texttt{a} directly from the definition of \texttt{+} above (this fact is recorded in core Lean as the lemma \texttt{Nat.add\textbackslash{}\_zero}).
\item
  The right side, \texttt{0 + a}, is \emph{not} immediate from the definition, since the recursion is on the second argument, not the first. It therefore needs its own small lemma, \texttt{Nat.zero\textbackslash{}\_add : 0 + a = a}, proved separately by induction on \texttt{a}.
\item
...
\item
  \href{https://lean-lang.org/doc/reference/latest/Tactic-Proofs/Tactic-Reference/}{\texttt{rw [Nat.add\textbackslash{}\_succ]}} uses the defining equation \texttt{a + succ k = succ (a + k)} to rewrite the left-hand side of the goal.
\item
  \texttt{rw [ih]} uses the induction hypothesis to replace \texttt{a + k} with \texttt{k + a} inside the goal.
\item
  \texttt{rw [Nat.succ\textbackslash{}\_add]} uses the equation \texttt{succ k + a = succ (k + a)} to rewrite the right-hand side, so both sides now read \texttt{succ (k + a)}, which are literally identical. \texttt{rw} closes the goal automatically once the two sides match syntactically.
\end{enumerate}
