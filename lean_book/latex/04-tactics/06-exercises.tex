...
\item
  \emph{\texttt{rw} rewrote \texttt{Nat.add\_\allowbreak{}succ} and closed the \texttt{succ} case of \texttt{my\_\allowbreak{}add\_\allowbreak{}comm}, but the same tactic failed on \texttt{a + b = b + a} before any induction. What changed between the two attempts?} Nothing about \texttt{rw} itself --- what changed is what is available to rewrite \emph{with}. Before induction there is no fact relating \texttt{a + b} and \texttt{b + a} at all; after \texttt{induction b}, the \texttt{succ} case comes with \texttt{ih : a + k = k + a} already in hand, giving \texttt{rw} something to substitute.
\item
...
\item
  \emph{\texttt{nat\_\allowbreak{}mul\_\allowbreak{}zero (n : Nat) : n * 0 = 0} closes by \texttt{rfl} alone. Would \texttt{0 * n = 0} also close by \texttt{rfl}?} No --- \texttt{Nat.mul} recurses on its \emph{second} argument, exactly like \texttt{Nat.add}, so \texttt{n * 0 = 0} is the base clause (immediate), while \texttt{0 * n = 0} needs an actual induction on \texttt{n}, for the same left/right-asymmetry reason \texttt{0 + n = n} did earlier in this chapter.
\item
...
\item
  Rewrite the \texttt{modus\_\allowbreak{}ponens} proof from Chapter 3 in tactic mode.
\end{enumerate}

Solutions: \hyperref[sec:14-appendix-solutions:03-chapter-4:1]{Appendix, Chapter 4}.

With definitions, propositions, and tactics in hand, we pause for a brief rigor check before diving into groups. Continue to \hyperref[sec:05-rigor-check:00-index:1]{Chapter 5: Rigor check}.
