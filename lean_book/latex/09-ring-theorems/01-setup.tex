...
``Let \((R, +, \times, 0, 1)\) be a ring.'' The \texttt{variable} fixes an arbitrary object of \(\mathbf{Ring}\); every theorem below is a statement about all rings at once. The recommended mental dictionary is just the standard ring notation --- \texttt{Rg.addGrp.op} \(= +\), \texttt{Rg.addGrp.id} \(= 0\), \texttt{Rg.addGrp.toGroup.inv} \(= -(-)\), \texttt{Rg.mul} \(= \times\), \texttt{Rg.one} \(= 1\) --- recovered by projecting along the

\end{mathreading}

that a qualified name like \texttt{Rg.addGrp.toGroup.inv a} is literally ``\(-a\) in the underlying additive group of \(R\).''
