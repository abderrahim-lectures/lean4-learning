...
\] using \(0 = 0+0\) and \texttt{left\_distrib}; adding \(-x\) and cancelling gives \(0 = x\). Conceptually this says the map \(x \mapsto a\cdot x\) is a group homomorphism of \((R,+)\), and homomorphisms send the identity to the identity. \(0\) absorbs because multiplication is additive in each argument.

\end{mathreading}

\textbf{Mathlib equivalent.} Where the book spends a full \texttt{have}/\texttt{congrArg}/\texttt{rw} derivation getting from \texttt{left\_distrib} and group cancellation to \(a\cdot 0=0\), Mathlib already proves this and gives it exactly the same name:

\begin{lstlisting}
example {R : Type*} [Ring R] (a : R) : a * 0 = 0 := mul_zero a
\end{lstlisting}

There is nothing to derive --- \texttt{mul\_zero} is proved once, generically, for every \texttt{Ring} (indeed every \texttt{MulZeroClass}), by essentially the argument above, and is then simply \emph{available} rather than re-derived at each use site.
