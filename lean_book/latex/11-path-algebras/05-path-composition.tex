...
\tightlist
\item
  If \texttt{q} is trivial (\texttt{Path.nil \_}, a path of length zero from \texttt{v} to \texttt{v}), then appending \texttt{p} before it just gives back \texttt{p} itself (there is nothing to append).
\item
