...
\tightlist
\item
  If \texttt{q} is trivial (\texttt{Path.nil \textbackslash{}\_}, a path of length zero from \texttt{v} to \texttt{v}), then appending \texttt{p} before it just gives back \texttt{p} itself (there is nothing to append).
\item
...
The previous section built \texttt{pathBetaAlpha : Path exampleQuiver 0 2} directly, one \texttt{Path.cons} at a time. Here it is again, built instead by \emph{composing} the shorter path \texttt{pathAlpha} with a fresh one-arrow path using \texttt{Path.append}. Reassuringly, both constructions produce the exact same path.
