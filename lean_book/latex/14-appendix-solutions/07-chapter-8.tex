...
\textbf{2. Why both \texttt{left\textbackslash{}\_distrib} and \texttt{right\textbackslash{}\_distrib} are needed}

Distributivity as usually stated, \(a(b+c) = ab+ac\), only lets you expand a product with the \emph{sum on the right}. Its mirror, \((a+b)c = ac+bc\), expands a product with the \emph{sum on the left}. If \texttt{mul} were known to be commutative, these two would be interchangeable --- just apply \texttt{comm} and use the other one. But a general \texttt{Ring} (unlike a \texttt{CommRing}) makes no such assumption. In fact, Chapter 9's proof of \texttt{mul\textbackslash{}\_zero} uses \texttt{left\textbackslash{}\_distrib}, while \texttt{mul\textbackslash{}\_zero\textbackslash{}\_left} (its mirror, an exercise) needs \texttt{right\textbackslash{}\_distrib} instead, precisely because there is no \texttt{mul\textbackslash{}\_comm} field to convert one into the other.

\textbf{3. \texttt{theorem mat2\textbackslash{}\_not\textbackslash{}\_comm : ∃ X Y : Mat2, Mat2.mul X Y ≠ Mat2.mul Y X}}
