...
- $t_1\, t_2$ — an **application**: applying $t_1$ to the argument $t_2$.
  ...
  applications of this form have appeared since Chapter 1's `double 5`).

This is the entire grammar. No numbers, no booleans, no `if`, no
recursion. As the next section shows, every one of those is
...
### α-conversion: bound names do not matter
...
This is called **α-conversion** (alpha-conversion). It is why, in Lean, one
never needs to worry that one proof uses `fun a => a` while
another uses `fun x => x`: they are the identical term, α-equivalent, and
...
as $(\lambda x.\, x + 1)\, 5 \to_\beta 5 + 1$. Python's `lambda` is,
however, a deliberately limited subset of $\lambda x.\, t$: the body `t`
must be a single *expression* — no `if` statements, no `for` loops, no
multiple statements, no `return`. Anything with real control flow requires
`def` instead, which is not the same kind of thing at all (`def` binds a
name via a statement; `lambda` produces a value). The actual untyped
λ-calculus has no such restriction, because none is needed: `if`, loops,
and recursion are, as the next section shows, just more terms built from
$\lambda$ and application, not separate language features bolted on top.
Where Python draws a line — "expressions get a terse `lambda`, everything
else needs `def`" — the λ-calculus (and Lean's `fun`, which imposes no such
restriction either) needs none. In the calculus, functions already
...
abstraction — it does not depend on $y$ at all, since the original body was
just $x$, renamed nothing. The second step substitutes $b$ for $y$ in a
body that does not mention $y$, so it discards $b$ and leaves $a$.
...
exactly the **currying** relied on silently since Chapter 1:
...
### Church–Rosser: reduction order does not matter (for the final answer)

If a term has more than one β-redex (a reducible sub-term
$(\lambda x. t)\, s$), which one is reduced first is a genuine choice.
The **Church–Rosser theorem**, however, guarantees that if a term reduces to a
normal form at all, that normal form is unique, regardless of the order in
which the reductions were taken to reach it. This is the theoretical bedrock
under a fact relied upon constantly without notice: Lean never has to
...
guarantees there is no wrong order, only faster or slower ones.

## Next

Continue to [Church encodings](02-church-encodings.md), which builds
