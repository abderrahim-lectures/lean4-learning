...
We already reused it for `∧`
([Chapter 3 §4](04-and-or-not.md)'s `⟨hp, hq⟩`), and now it builds an
...
claiming exists, and `p` is *why* it satisfies the property. They are
packed together because that's exactly the two pieces of data an
...
uniformly for an *arbitrary* `n`. That means induction, which we haven't
introduced yet. [Chapter 4](../04-tactics/00-index.md) covers the tactics
