...
require `show` first to reveal the `+`/`*`-form of the goal before `rw` can find
...
of even integers, $2\mathbb{Z} = \{\, m \mid \exists k,\ m = 2k \,\}$. This
is the image of multiplication-by-$2$, and equivalently the principal ideal
...
*generically*, using [`Submodule.span`](https://loogle.lean-lang.org/?q=Submodule.span). Thus the three
hand-written closure proofs of `evenSubmodule` (`zero_mem`/`add_mem`/`smul_mem`) become a
...
instead of the custom `zero_mem`/`add_mem`/`smul_mem` triple of the book for
