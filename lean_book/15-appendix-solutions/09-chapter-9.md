...
[← Chapter 8](08-chapter-8.md) | [Index](00-index.md) | [Next: Chapter 10 →](10-chapter-10.md)
...
We reuse `boolXorGroup` from the exercise of Chapter 7 as the additive group
(`+` = XOR, `0` = `false`), and add `∧` (Boolean and) as multiplication with
`1 = true`. As with `assoc` in Chapter 7, every axiom is a finite check over
...
assumption. In fact, the proof of `mul_zero` in Chapter 10 uses `left_distrib`,
