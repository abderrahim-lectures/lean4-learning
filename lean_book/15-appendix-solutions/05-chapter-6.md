## Chapter 7: Rigor check — structures, universes, and equality

[← Chapter 6](04-chapter-5.md) | [Index](00-index.md) | [Next: Chapter 8 →](06-chapter-7.md)
...
definition (this is the same asymmetry Chapter 6 relied on for
...
This is the same reason `0 + n = n` required real induction in Chapter 6.
...
`intGroup : Group Int` `def` from Chapter 8. Registering something as an
`instance` instead of a plain `def` only changes *how the elaborator of Lean
...
could rebuild the Russell paradox inside Lean (a self-referential type such as
"the type of all types not containing themselves"). The kernel of Lean avoids
...
`0 + n = n` from Chapter 6. The recursion of `Nat.add` never touches a variable
...
Both instances are one field-by-field build, exactly the style of `intGroup`
(Chapter 8, Section 3) minus the two inverse fields: each proof obligation is a
...
one). The proof of `monoid_id_unique` is `id_unique` (Chapter 9, Theorem 1) copied
...
"prove once, get it for free" payoff Chapter 8, Section 6 promises for `Group`.
