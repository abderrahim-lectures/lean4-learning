...
group `Rg.addGrp.toGroup`), it's enough to show that
...
`left_inverse_unique`, or induction is needed, since there's nothing to
...
`neg_one_mul` is a genuine theorem (propositional, needing proof) precisely
