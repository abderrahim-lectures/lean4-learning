...
$h' : t(a) = w$ enforcing head-to-tail composability — the type-level
encoding of "$\circ$ is only defined on composable pairs." Because the
composability constraint lives *in the type*, an ill-formed composite simply
fails to typecheck, exactly the dependent-type payoff from Chapter 1.
...
$s(\beta) = 0$, i.e. $1 = 0$, a false equation with no proof — the type
system refusing to compose non-composable arrows, which in a category is the
