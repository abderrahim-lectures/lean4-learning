...
1. *The composability check of `Quiver.Path` (the source of an arrow must match the
   current endpoint of the path) is enforced by Lean at compile time. What
   ...
   all.
2. *A quiver with a genuine cycle (the `gamma` of Exercise 1) has infinitely
   ...
   terms inhabit that one type, one per lap. This is exactly why the
   path algebra $kQ$ of a quiver is typically infinite-dimensional once the
...
2. Prove `theorem append_nil_left {V A} {Q : Quiver V A} {u v} (p : Path Q u v) : Path.append (Path.nil u) p = p`
   ...
   (`Path.cons`) cases as in the `induction` examples of Chapter 4.
3. (Optional, harder) Sketch, in comments, no need to complete the Lean,
   ...
   would need to contain to satisfy the fields of `Ring` from Chapter 8.
